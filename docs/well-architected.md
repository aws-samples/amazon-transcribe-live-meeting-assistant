...
- **Automated Security Scanning in CI/CD**: The repository integrates the AWS [Sample Security Review Tool (SRT)](https://github.com/aws-samples/sample-security-review-tool) — a multi-scanner harness covering Bandit (Python), Semgrep (multi-language SAST), Checkov (CloudFormation IaC), Syft (SBOM/dependency inventory), and an AWS-resource security-matrix. SRT runs locally via `make srt` and automatically in GitLab CI on merge requests targeting `develop` ([`.gitlab-ci.yml`](https://github.com/aws-samples/amazon-transcribe-live-meeting-assistant/blob/develop/.gitlab-ci.yml) — see [Security Scanning](./security-scanning.md)). Review decisions live in `.srt/suppressions.json`, which is tracked in git via a negative-gitignore rule, so the pipeline shares the team's curated decisions and fails the merge request when new HIGH-priority findings appear. SRT's working database, `.srt/issues.json`, is generated per run and is seeded from the tracked decisions on a fresh checkout.
- **Documented Threat Model**: The repository includes a dedicated `threat-modeling/` directory with architecture, threat-analysis, and risk-assessment artifacts.
