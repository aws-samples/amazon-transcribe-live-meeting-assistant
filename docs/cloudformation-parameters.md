...
| Parameter | Description | Default | Allowed Values |
|-----------|-------------|---------|----------------|
| TranscriptionEngine | Deploys the on-demand ASR + diarization stack. Meetings still use Amazon Transcribe until an admin switches the default on the ASR Config page or a Stream Audio meeting picks the engine | AmazonTranscribe | AmazonTranscribe, MicrovmAsr |
...
speaker model, its operating point measured with `scripts/calibrate.py`. The two
runtime switches (deployment default engine, Virtual Participant voice separation) live
on the ASR Config admin page and take effect on the next meeting with no stack update.
