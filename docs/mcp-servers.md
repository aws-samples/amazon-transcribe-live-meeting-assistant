...
Installed MCP servers are account-wide — they apply to every meeting — so installing, updating and uninstalling them is reserved for members of the **Admin** Cognito group. The `installMCPServer`, `updateMCPServer` and `uninstallMCPServer` GraphQL operations accept Admin callers only, and the `MCPServerManager` Lambda function independently re-checks the caller's group membership before it acts. Signed-in users who are not in the Admin group see the installed servers read-only: the Public Registry and Custom Server tabs and the per-server Update/Uninstall actions are not shown. The per-user connection details on the **Hosted MCP Access** tab remain available to every user.
...
- **Servers already installed stay installed.** Every row in the MCP servers table is kept as-is and the Strands agent keeps loading those servers. What changes is who can alter the set: from this release `installMCPServer`, `updateMCPServer` and `uninstallMCPServer` accept callers in the **Admin** group. If a server was installed by someone who is not in that group, add them to the Admin group (User Management page) or have an administrator make future changes.
