...
- **Context aware meeting assistant** - Powered by the Strands Agents SDK with Amazon Bedrock, the meeting assistant uses the live transcript as context for fact-checking and follow-up questions. Optionally integrate a Bedrock Knowledge Base for answers from your trusted document sources. To activate the assistant, just say "_Okay, Assistant_," or enter your own question in the chat UI.
...
If you want LMA to use your own trusted documents to power the context aware meeting assistant, you can optionally use a Knowledge Base for Amazon Bedrock. You choose to have LMA either:

1. Automatically create a new Bedrock knowledge base and populate it from documents in an S3 bucket in your AWS account, or from publicly accessible Web pages. Be prepared to specify your document location when you deploy LMA. Be prepared to specify your document location when you deploy LMA.
...
1. For **MeetingAssistService** choose `STRANDS_BEDROCK` (default, no knowledge base), `STRANDS_BEDROCK_WITH_KB (Create)` to auto-create a knowledge base, or `STRANDS_BEDROCK_WITH_KB (Use Existing)` to use your own
   1. If you select `STRANDS_BEDROCK_WITH_KB (Use Existing)`, then:
   ...
   1. If you select `STRANDS_BEDROCK_WITH_KB (Create)`, a new knowledge base is created for you automatically:
...
- LMA meeting assistant resources powered by the Strands Agents SDK with Amazon Bedrock.
...
For Meeting Assist, refer to the [Meeting Assist README](./lma-meetingassist-setup-stack/README.md) for additional information.
...
Meeting Assist is powered by the Strands Agents SDK with Amazon Bedrock. You can optionally create or use a Bedrock Knowledge Base for document search – see [Amazon Bedrock pricing](https://aws.amazon.com/bedrock/pricing/) for more.
...
- To customize the Meeting Assist capabilities, see the [Meeting Assist README](./lma-meetingassist-setup-stack/README.md).
...
The Live Meeting Assistant sample solution offers a flexible, feature-rich, and customizable approach to provide live meeting assistance to improve your productivity during and after meetings. It uses Amazon AI/ML services like Amazon Transcribe, the Strands Agents SDK, Knowledge Bases for Amazon Bedrock, and Amazon Bedrock LLMs to transcribe and extract real-time insights from your meeting audio.
