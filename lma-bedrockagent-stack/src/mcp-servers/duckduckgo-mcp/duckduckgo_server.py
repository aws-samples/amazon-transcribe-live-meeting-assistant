...
async def search(query: str, ctx: Context, max_results: int = 10) -> str:
    ...
    """
    logger.info(f"🔍 Search request received: query='{query}', max_results={max_results}")
    try:
...
async def fetch_content(url: str, ctx: Context) -> str:
    ...
    Args:
        ...
        ctx: MCP context for logging
    """
    logger.info(f"🌐 Fetch content request received: url='{url}'")
    try:
        response = await fetcher.fetch_and_parse(url, ctx)
        logger.info(f"✅ Content fetch completed: {len(response)} characters")
        return response
    except Exception as e:
...
def main():
    ...
    logger.info("📡 Server will listen on host: 0.0.0.0, port: 8000")
    logger.info("🌐 Server URL: http://localhost:8000/mcp")
    logger.info("🔧 Available tools: search, fetch_content")
