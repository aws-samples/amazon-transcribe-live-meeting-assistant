...
class TestRevokeKey(unittest.TestCase):
    ...
    def test_revoke_key_success(self, mock_dynamo):
        ...
        mock_table.query.return_value = {
            "Items": [{"KeyHash": "hash123", "KeyPrefix": "lma_abcdef12"}]
        }
        mock_dynamo.Table.return_value = mock_table

        event = make_event(
            "revokeMCPApiKey", username="alice", arguments={"keyPrefix": "lma_abcdef12"}
        )
        result = index.handler(event, None)
    ...
    def test_revoke_key_not_found(self, mock_dynamo):
        ...
        event = make_event(
            "revokeMCPApiKey", username="alice", arguments={"keyPrefix": "lma_wrongpfx"}
        )
        with self.assertRaises(Exception) as ctx:
            index.handler(event, None)
    ...
    def test_revoke_key_no_keys(self, mock_dynamo):
        ...
        event = make_event(
            "revokeMCPApiKey", username="alice", arguments={"keyPrefix": "lma_anything"}
        )
        with self.assertRaises(Exception) as ctx:
            index.handler(event, None)
        self.assertIn("Key not found", str(ctx.exception))
