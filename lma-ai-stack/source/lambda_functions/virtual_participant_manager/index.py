...
class VirtualParticipantManager:
    ...
    def end_virtual_participant(
        self, vp_id: str, end_reason: str, ended_by: str = None
    ) -> Dict[str, Any]:
    ...
    def cancel_eventbridge_schedule(self, vp_id: str) -> bool:
        ...
        try:
            ...
            logger.info(
                f"Attempting to delete schedule {schedule_name} from group {schedule_group_name}"
            )

            scheduler.delete_schedule(GroupName=schedule_group_name, Name=schedule_name)
            ...
            logger.warning(
                f"Schedule for VP {vp_id} not found (may have already been executed or deleted)"
            )
