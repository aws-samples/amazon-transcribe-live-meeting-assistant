...
- **Speaker-diarization operating point, measured rather than guessed** — a live single-speaker meeting on the MicroVM ASR engine produced eight speaker identities for one person. Replaying the production speaker registry over that meeting's real embeddings, with a second two-person recording as a different-speaker control, showed the cause was the similarity threshold, not the model: with TitaNet, different speakers never scored above 0.107 while the same speaker scored 0.25–0.5, so the inherited default of 0.5 split almost every utterance. `AsrSpeakerThreshold` now defaults to **0.2** and `AsrMinSegmentMs` to **2500**, which takes that meeting from 8 identities to 2 and resolves a two-speaker recording to exactly 2 with 100% attribution purity. The threshold is model-specific, so `source/catalog.json` now records a measured `recommendedThreshold` per speaker model, and WeSpeaker CAM++ and ResNet293 (both Apache-2.0) are available as alternatives with their measurements attached — ResNet293 looked perfect on single-speaker audio yet scored 0.54 between two *different* people, which is why a same-speaker-only test is not sufficient evidence. Also adds an off-by-default `require_corroboration` policy that withholds the first unmatched embedding; it rescues a mis-set threshold but costs attribution purity when the threshold is right, so it is opt-in. `AsrMaxSpeakers` stays at 0 (discover): how many people share a microphone is not knowable in advance, and a cap bounds the symptom rather than fixing the operating point.

- **A fully permissive model stack for the on-demand ASR engine** — the default models are licensed under the NVIDIA Open Model License (ASR) and CC-BY-4.0 (speaker embedding), which some deployments cannot use. `AsrModelId` now also offers `zipformer-streaming-en-2023-06-26-int8` (Apache-2.0, icefall), and `AsrSpeakerModelId` now exposes the two WeSpeaker embedders already in the catalog (`wespeaker-en-cam++-lm`, `wespeaker-en-resnet293-lm`, Apache-2.0) — previously only `titanet-small` and `none` were selectable, so the WeSpeaker entries could not actually be chosen from the main stack. Combining the Zipformer with a WeSpeaker model gives an Apache-2.0-only stack from runtime to weights. Both are the *less* accurate choice on the audio measured so far (the Zipformer is trained on read speech, and both WeSpeaker models separated speakers worse than TitaNet), so this is a licensing option rather than a quality one — calibrate after switching. Whisper / Distil-Whisper remain unavailable because this engine is streaming-only by construction; see the new "Not included" section in the docs for what enabling them would take and why the WhisperX-style hybrid belongs on the batch path instead.

- **Model selection is explicit: the Custom-model parameters are gone** — ten CloudFormation parameters (`AsrModelUrl`, `AsrModelSha256`, the four `AsrModel*File` names, `AsrSherpaOnnxVersion`, `AsrOnnxruntimeVersion`, `AsrSpeakerModelUrl`, `AsrSpeakerModelSha256`) and the `Custom` option on both model parameters have been removed. A model needs its checksum pinned, its file names known, its runtime versions matched and — for a speaker model — its operating point measured, which is not work to do in a console text box while an engine is still being proven. Every model is now a curated entry in the ASR stack's `catalog.json`, validated at resolve time, and adding one is a four-step repo change documented in the docs. The unmeasured-model guardrail still applies and reads better for it: "unmeasured" now means a catalog entry with no `measured` note rather than a pasted URL.

- **Speaker labels no longer hand a placeholder to a real person.** A voice's label is now the channel's own name plus the engine's voice id (`Other Participant (spk_0)`, `Other Participant (spk_1)`) — the same format the Amazon Transcribe path uses, so a transcript reads identically whichever engine produced it. Previously the first voice heard on a channel inherited that channel's name alone, which for the tab is the placeholder "Other Participant" — so a meeting whose two remote speakers were separated *correctly* produced a transcript reading `Other Participant` beside a numbered speaker, and a reviewer reasonably concluded the two had been merged into a leftover bucket. Clustering that meeting's embeddings independently of the labels showed two voices 0.34 apart with the labels matching the clusters exactly: the diarization was right and the naming was misleading. Transcripts are read by people, exports and summarisation models alike, so a name that looks like a bucket is a correctness problem, not a cosmetic one. The suffix is what carries the distinction, and it degrades well: a partial has no known speaker yet, so it shows the bare channel name and gains the suffix once the label is real.

- **Speaker turns inside one utterance are now split into separate transcript rows** — endpointing closes an utterance on silence, so two people speaking without a gap between them shared one row and one speaker label (the "one speaker per utterance" limitation). The engine now bakes **pyannote segmentation 3.0** (MIT, 6 MB ONNX, redistributed ungated by k2-fsa so no Hugging Face token is needed), runs it over each closed utterance, and emits one `final` per detected turn — each embedded and identified separately, and cut at a word boundary so no word lands in two rows. Three rules keep it from making transcripts worse: a change must persist 700 ms to count (a back-channel "mhm" does not split a sentence), analysis windows overlap by half with only the trusted middle of each used (a window seam is never mistaken for a turn), and a part too short to embed is merged back into its neighbour. Supporting this, the streaming recogniser now reconstructs word timings from sherpa's per-token output, and a segment without word timings is never split. Validated end to end against k2-fsa's own two-speaker sample: the turn is found at 7.89 s and the rows come back as two speakers with the words divided evenly. Off for a bundle with no segmentation model, or when **Split rows on a speaker change** is unticked on the ASR Config page (no rebuild). Cutting mid-utterance, so a speaker change rather than a pause is what separates rows, arrived later in this branch — see the live-turn-cut entry above.

- **Diarization calibration — measure the speaker threshold from your own meetings instead of inheriting a number** — the similarity threshold that separates "same person" from "different people" is a property of the speaker-embedding model, not a constant: with TitaNet two different speakers never exceeded 0.107, while WeSpeaker ResNet293 scored 0.54 between two *different* people on the same audio. A shipped default therefore cannot be right for a model nobody measured, including one a deployment supplies itself. **Configuration ▸ ASR Config ▸ Calibrate from a two-channel recording** now derives the operating point from a WAV you upload, where each channel carries one speaker: within-channel pairs are the same person and cross-channel pairs are definitely different people, which is exactly the comparison a threshold has to get right. Taking the file rather than a meeting ID means a sample can come from a rehearsed recording, a meeting you downloaded, or a public corpus — and the page works whether or not a suitable meeting happens to have been recorded. The upload is embedded in memory and discarded, never written to S3 or a transcript. The admin page documents both ways to make one: play a recording of someone else through your laptop speakers while talking into the mic and capture it with Stream Audio (whose two channels are exactly system audio and microphone), or merge two per-speaker files from a public corpus with a given ffmpeg line. Note this is the opposite of what most pipelines do with stereo — pyannote and friends downmix to mono, while this engine splits the channels and processes each independently, which is what makes channel identity usable as a label. The transcriber finds stretches where one channel clearly dominates (dominance, not silence, so cross-talk is excluded), embeds up to 12 per channel on a MicroVM in a new embed-only session mode, and places the threshold inside the gap between the two distributions — nearer the different-speaker side, and above the highest different-speaker score actually observed, so no pair the calibration saw would have merged. It also recommends a minimum utterance length when short segments measurably embed worse, the measured cause of phantom speakers. When the distributions overlap it **refuses to return a number** and says why, because a threshold that fragments or merges with equal confidence is worse than none. Nothing is applied automatically: a run reports, an admin reviews, Save applies. Admin-only and single-flight, and a one-sided recording is refused before a MicroVM is launched.

- **Speaker labels are withheld for a speaker model whose operating point was never measured** — every speaker model in the ASR catalog now carries a `measured` note recording what was actually observed on real audio. An entry with no such note — a model added but not yet characterised — makes the stack report `SpeakerModelMeasured=false` and meetings are transcribed with channel labels — with the reason logged — until a calibration is applied or an admin sets a threshold. Working-looking diarization built on a guessed threshold is worse than none, because the transcript looks plausible while attributing speech to the wrong people.
...
- **On-demand ASR + speaker diarization on AWS Lambda MicroVMs — an alternative to Amazon Transcribe streaming** — A new `lma-asr-microvm-stack` (created when the main stack's `TranscriptionEngine` parameter is set to `MicrovmAsr`) runs `sherpa-onnx` streaming speech recognition **and** online speaker diarization in one process on a MicroVM launched per meeting and terminated when it ends. Its purpose is per-voice speaker labels: today a Stream Audio or Desktop Capture meeting is labelled by audio **channel**, so several people sharing a conference-room microphone become one speaker. Because the speaker label is derived from the same samples and the same segment boundaries the recognizer produced, the label and the text cannot drift apart the way a separate diarizer aligned against a separate ASR service can. It also gives deployments a transcription engine that is not Amazon Transcribe, with the model swappable by CloudFormation parameter (`AsrModelId`, plus `Custom` with your own URL + SHA256 and runtime pins) for licensing or accuracy reasons.

  Opt in per meeting: tick **Enable speaker diarization** on the Stream Audio page (which also makes the browser capture at the models' native 16 kHz), pass `--diarization 1` to the macOS or Windows Desktop Capture App, or send `enableDiarization` in a custom client's `START` frame. Everything else is unchanged: the engine emits the same `ADD_TRANSCRIPT_SEGMENT` events, so transcripts, summaries, the meeting assistant, sharing and search work as before, the Fargate transcriber task keeps its existing size (inference happens in the MicroVM), and a meeting whose MicroVM cannot be acquired falls back to Amazon Transcribe automatically. Deploying the stack changes nothing on its own — meetings still use Amazon Transcribe unless a client asks for diarization.

  ⚠️ A meeting transcribed by this engine does **not** go through Amazon Transcribe, so content redaction/PII, custom vocabularies, custom language models, language identification and Call Analytics do not apply to it, and the default model is **English only**. Requires a region where AWS Lambda MicroVMs is available (not GovCloud). Word error rate, diarization error rate and MicroVM launch latency have **not yet been benchmarked** — measure them in a dev stack before offering the engine to users. See [On-demand ASR & Speaker Diarization](docs/microvm-asr.md).

- **ASR Config admin page — tune diarization without a stack update** — the diarization operating point is empirical and model-specific, so iterating on it through 20-minute CloudFormation updates was impractical. A new **Configuration ▸ ASR Config** page (admin only, following the Nova Sonic config pattern: DynamoDB + AppSync + validating Lambda resolver) holds runtime overrides for the similarity threshold, minimum utterance length, speaker cap, endpointing, and whether the engine is the deployment default. Every field is optional — blank means "use the CloudFormation parameter" — so there is no default record to drift, and a failed read falls back to the deployment's own parameters rather than failing the meeting. The transcriber reads the record once per meeting (cached 30s) and sends it as per-session config, which required adding `min_segment_ms` and `require_corroboration` to the WebSocket protocol so that all four diarization knobs are now changeable at runtime instead of only at image-build time. Stream Audio additionally asks **Speakers on this side** when diarization is enabled, mirroring the question Upload Audio already asks, because only the participant knows how many people share their microphone; a value there overrides the deployment-wide cap for that meeting.
...
- **Speaker labels are now right when the text first appears, and a speaker change closes a row without waiting for a pause.** Three changes to the MicroVM ASR engine, all on by default:
  ...
  **Partials name nobody.** A partial used to carry the last identified speaker as a provisional label, so the person who had just *stopped* talking was named against the words of whoever started, until the final corrected it a second or two later. Reviewers watching the live transcript reported the label "transforms" mid-sentence and read it as the diarization being unstable, when the final labels were in fact correct. Partials now carry no speaker, so the row shows the plain channel name until the label is known — matching the Amazon Transcribe path, which never had labels on partials to begin with.

  **A confirmed speaker change closes a row (`AsrLiveTurnCut`).** Endpointing silence used to be what separated speakers, so two people talking without a gap shared one row and one label until somebody paused. The speaker change is now the primary boundary and the pause is only a backstop. While a segment is open the engine re-runs the turn detector about once a second over the audio since the last cut and closes a row as soon as a boundary has `minSegmentMs` of audio before it and `ASR_MIN_TURN_MS` after it — the second condition being what stops a back-channel from producing one-word rows. Deliberately done in the diarization layer rather than by forcing the recognizer to endpoint early: resetting the decoder mid-utterance would cost it the left context it decodes with, and text quality is already the weaker half of this engine.

  **A row closes after 20s regardless (`AsrMaxOpenSegmentMs`).** With no speaker change at all a monologue would otherwise sit in the live transcript as one unlabelled block for as long as somebody kept talking. Mirrors the equivalent bound on the Amazon Transcribe path.

  All three are tunable on the ASR Config admin page without a stack update. Known limitation: a committed row is not rewritten if the streaming decoder later revises its text — only audio at least `ASR_MIN_TURN_MS` in the past is ever committed, by which point revisions are rare, but a late one is lost rather than corrected.

- **Fixed: omitting `speaker_threshold` from a session config silently overrode the bundle's calibrated value.** The wire field defaulted to `0.5`, so any client that simply did not send it got 0.5 rather than the operating point measured for the deployed model bundle. It is now optional, meaning "use the server's calibrated value". Also fixed `build_diarization_config` computing `split_on_speaker_change` and then dropping it, so `ASR_SPLIT_ON_SPEAKER_CHANGE=0` was ignored.
...
- **A meeting on the MicroVM engine buffered its entire audio stream in memory, and would OOM the transcriber task.** `onBinaryMessage` writes every frame into `audioInputStream`, but `startTranscribe` is that stream's only consumer and does not run on this path — so nothing ever drained it and backpressure was ignored. At the 16 kHz stereo this engine negotiates that is 64 kB/s, about 230 MB an hour, in a task with 1 GiB shared by every other meeting on it: a four-hour meeting would have reached ~920 MB and taken its neighbours down with it. Measured directly (10 minutes of frames buffered 38.4 MB undrained, 0 MB drained). The stream is now drained as soon as the MicroVM engine starts successfully — after, not before, so audio captured during the ~9 s MicroVM launch is still available to the Amazon Transcribe fallback.

- **The ASR Config page's "make this engine the default" switch could not affect any web meeting.** Stream Audio sent an explicit `asrEngine` on every meeting, and an explicit client value beats the deployment default by design — so the admin switch was inert for the only client that had no other way to reach the engine, contradicting both its own description and the docs. The engine is now sent only when the user actually moves the radio.

- **An admin's `maxSpeakers` cap never applied to a Stream Audio meeting.** The precedence used `??`, which treats `0` as a deliberate choice, and `0` is exactly what a blank "Speakers per channel" field sent. Extracted as `resolveMaxSpeakers` with tests: a falsy client value now means "no opinion", and the UI omits the field entirely when blank.

- **The calibration upload put a Cognito access token in the URL query string.** That CloudFront distribution has access logging enabled, so tokens were persisted to the log bucket (and the server logs `request.url` at INFO). Moved to an `Authorization` header, which the route already accepted — the WebSocket route has no choice, a browser cannot set headers on a WebSocket, but this is a plain `fetch`.

- **`updateAsrConfig` was callable by any signed-in user**, letting a non-admin change the deployment's transcription engine and every diarization parameter. Now scoped to the `Admin` group, matching the calibration route's own check.

- **The two new Lambdas had no declared log groups**, so they logged to implicit `/aws/lambda/<name>` groups with no retention and no customer-managed-key encryption. Each now has a stack-scoped group with `RetentionInDays` and `KmsKeyId`; a stack-scoped name is used because a group named after the function already exists wherever it has run, and declaring that name would fail an update on a live stack.

- **The ASR Config page told a transcription-only deployment that the engine was not deployed.** It gated on `AsrDiarizationAvailable`, which is false for a bundle with no embedder even though the engine is deployed and configurable. Now gates on `AsrEngineAvailable`, and says plainly when a bundle carries no embedder.

- **This branch's CHANGELOG entries were filed under the released `## [0.3.7]`** (the `v0.3.7` tag exists), recording never-shipped work against a shipped version, and `[Unreleased]` had two `### Fixed` headings with additions orphaned under one of them. Eight entries moved to `[Unreleased]` and the headings consolidated; entry count verified unchanged at 129. Two entries that described behaviour later superseded in the same branch were corrected rather than left contradicting the code.

- **`THIRD-PARTY-LICENSES.txt` documented removed parameters and none of the added weights.** It referenced `AsrModelId` / `AsrSpeakerModelId` / "the Custom model parameters", all replaced by `AsrModelBundle`, and listed neither Parakeet, FastConformer, TitaNet-large, 3D-Speaker, pyannote segmentation nor Silero VAD. Rewritten around bundles, with redistributability called out per model — this is the file that matters most for a repo mirrored to a public AWS sample.

- **25 Python files in the ASR stack were missing the copyright/MIT header**, inconsistent with files added later in the same tree.

- **`SpeakerNameRegistry`'s docstring described a numbering scheme the code no longer implements**, and `channelToLabelSuffix` was left dead after the label format changed. Also removed a duplicate `ExpandableSection` import and a `--diarization` flag the macOS app documents but no longer parses.


- **A transient failure downloading model weights no longer fails the ASR image build.** The fetch runs inside `docker build`, so a single `HTTP 503` from the public release host failed the image, the nested stack, and the whole stack update — a nine-minute deployment ending in a rollback (observed). Downloads are now retried five times with backoff on the transient classes (503/502/500/504/429/408 and dropped or truncated connections), while a 404 or a checksum mismatch still fails immediately and loudly, because those mean the model selection is wrong rather than unlucky.
...
- **Per-channel speaker identification for WebSocket streaming sessions** — the [Stream Audio](docs/stream-audio.md) tab and the [Desktop Capture App](docs/desktop-capture-app.md) can now ask Amazon Transcribe to tell apart the individual voices *within* an audio channel, enabled **independently per channel**: turn it on for the incoming/system audio when a captured call has several remote participants, for the microphone when several people share one mic (a conference room), for both, or for neither. Until now each channel got a single speaker name, so everyone on the far end of a captured call appeared as one speaker. Each distinct voice is appended to the channel's existing speaker name as `(spk_0)`, `(spk_1)`, … — e.g. `Other Participant (spk_0)` — so client-supplied names are preserved rather than replaced, and the labels flow through the live transcript (which now splits turns per voice), the transcript download, the meeting summary and the Meeting Assistant. Configured per meeting in the client's own settings — checkboxes on the Stream Audio tab, **Settings ▸ Speaker identification** in the desktop app (plus `--diarize-system` / `--diarize-mic` for CLI use) — and sent to the transcriber when the session starts. **Off by default everywhere**; the new `ShowSpeakerLabel` CloudFormation parameter sets a deployment-wide default for clients that do not send their own choice. There is no additional Amazon Transcribe charge, and the two channels are still billed as a single stream. Accuracy is best with five or fewer voices per channel; speakers are numbered per channel starting at `spk_0`, and the labels are numbers rather than names — for real participant names use the Chrome Extension or a Virtual Participant, which read them from the meeting itself.

  Transcript segments are **split where the speaker changes**, which matters because a single Amazon Transcribe result routinely spans several turns — in natural conversation there is no pause to break on, so a 30-second result containing four turns is normal. Word-level labels are noisy (a single word flips mid-utterance, and punctuation carries no label), so a run of same-speaker words must reach both `DIARIZATION_MIN_RUN_WORDS` (default 3) and `DIARIZATION_MIN_RUN_SECONDS` (default 1.0) to become its own segment; shorter runs are merged into the neighbouring turn without moving any transcript text. The defaults were fitted to a real two-speaker recording where spurious runs measured 1-2 words / 0.1-0.9 s against real turns of 6-42 words / 1.2-13.4 s. One consequence: a genuine one- or two-word interjection is attributed to the speaker around it. Every final result on a diarized channel logs a `[DIARIZATION]` line at INFO with the raw runs, the resulting segments and how many were absorbed, so the thresholds can be re-tuned from real meetings.

  An in-progress segment is additionally **bounded at 20 seconds** (`DIARIZATION_MAX_SEGMENT_SECONDS`). Amazon Transcribe caps a result near 30 seconds and never labels a partial result, so following its boundaries alone left the live transcript showing one unlabelled block for up to half a minute before it split. Each result is now chunked into windows, and a window whose audio is already in the past is emitted as final while Transcribe still considers the result partial. A window boundary can fall mid-turn, producing two adjacent segments for the same speaker, which the UI merges on display.
...
- **Dependency updates across every stack**, landed as weekly grouped Dependabot batches — the UI, WebSocket transcriber, Virtual Participant backend, browser extension, tooling manifests and the boto3/botocore/cfn-lint/aws-sam-cli family — plus advisory fixes for `js-yaml`, `dompurify` and `@hono/node-server`, with no build regressions. The Cognito stack's Lambda runtime also moved from `python3.11` to `python3.12`. The two advisories deferred in 0.3.6 (fastify 3.x→5.x in the transcriber, and the `urllib3<2` pin held for boto3 runtime compatibility) remain deferred for the same reasons.


### Fixed
- **The macOS Desktop Capture App no longer quits itself after about an hour.** Sign-in produced a Cognito refresh token that the app then discarded, so nothing could renew the ~1 hour access token — and once it expired the transcriber rejected the WebSocket connection and the app called `exit(1)`, disappearing from the menu bar. Recovering meant quitting and relaunching, and any meeting in progress was lost. The app now keeps the refresh token and renews the session in the background before it expires (on a wall-clock schedule, so it survives the Mac sleeping), renews on demand when you start a recording after a long idle or a wake from sleep, and retries once if the server rejects a token anyway. Long meetings and a Start hours after signing in both work without interruption. If renewal genuinely cannot succeed — a changed password, an administrator sign-out, or a session older than the refresh-token lifetime — the app now **stays running** and shows "Your session expired and could not be renewed. Please sign in again." with the sign-in form, instead of vanishing. The headless CLI still exits non-zero, as before. See [issue #535](https://github.com/aws-samples/amazon-transcribe-live-meeting-assistant/issues/535).

- **The Windows Desktop Capture App no longer loses the meeting after about an hour.** Same root cause as the macOS fix above: sign-in produced a Cognito refresh token that the app discarded, so nothing could renew the ~1 hour access token, and once it expired the transcriber rejected the WebSocket connection. The Windows app never quit itself, but it ended the recording and demanded a fresh sign-in — mid-meeting, with no way to resume. It now keeps the refresh token and renews the session the same way the macOS app does: in the background before the token expires (re-checked on resume from sleep or hibernation), on demand when you start a recording after a long idle, and once more if the server rejects a token anyway. If renewal genuinely cannot succeed — a changed password, an administrator sign-out, or a session older than the refresh-token lifetime — the tray app shows "Your session expired and could not be renewed. Please sign in again." with the sign-in form; the headless CLI now exits non-zero (it previously exited 0 even on an auth failure). Like the macOS fix, the Windows client also stopped sending the (empty) `refresh_token` WebSocket parameter and now redacts tokens from connection-error logs. See [issue #535](https://github.com/aws-samples/amazon-transcribe-live-meeting-assistant/issues/535).

- **The macOS Desktop Capture App no longer announces a failed session as a finished meeting.** When a recording ended because the session could not be renewed (the path above), the app still posted "Recording stopped — your meeting is being processed", with a click-through link to a meeting that was never created, because every teardown — clean stop or fatal error — passes through the same intermediate state and the notification was decided from that state alone. The outcome is now taken from the state the session actually settles in, so a dead session reports a failure and a clean stop is unchanged. The Windows app was never affected.
...
- **The Virtual Participant now leaves when a Teams meeting ends.** Teams removes the participant-count badge when the host ends the meeting, and the watchdog treated a missing badge as "unknown — never leave", so the VP kept recording video and holding a voice session until its maximum duration was reached. It now recognises Teams' post-meeting screen directly, and a badge that stays missing is bounded rather than ignored — while a brief disappearance (collapsed roster, content share) is still absorbed without ending a live meeting. Affects all launch types.

- **Speech is no longer transcribed twice.** Meeting audio was reaching the transcription sink by two separate routes, so Amazon Transcribe received every utterance twice and transcribed both — "Jack and Jill, Jack and Jill went up…". Human speech was affected on all platforms; the assistant's own speech was not, because it only ever had one route. There is now a single route, and a test asserts it stays that way.
- Added diagnostic logging for the Virtual Participant's audio path, visible in CloudWatch: the browser's requested and applied microphone settings, capture continuity (how much audio the browser actually captured per second of wall clock), the negotiated audio codec and bitrate, packet loss and jitter as reported by the far end, which captured track is transmitted and whether the browser's echo canceller is running on it, and which PulseAudio device each process is capturing from. This is groundwork for an unresolved report of crackly voice assistant audio on Microsoft Teams (the assistant sounds clean on Zoom) — that report remains open, and these measurements have so far ruled out several suspected causes rather than identified one.
- Reduced Virtual Participant CPU use: the avatar no longer redraws a full-resolution canvas on every frame when a meeting client requests an exact video size (Teams does), and the audio buffers were widened from 20ms to 80ms — still well below conversational latency.

- **Transcription and recording could go completely silent part-way through a meeting.** Meeting audio reached the transcription sink through a PulseAudio loopback, which does not keep a null sink out of the daemon's idle/suspend state — so the sink went digitally silent (a live Zoom session recorded 50 seconds at peak amplitude zero) while the audio it was supposed to be carrying was still flowing elsewhere. The application now holds an active playback stream on that sink instead, which both delivers the audio and keeps the sink running. Affects all launch types.

- **The meeting audio recording never contained the voice assistant's spoken replies.** The recording was written from the meeting-only audio source, which deliberately excludes the assistant so the voice assistant never hears itself — so the assistant's side of every conversation was missing from every audio recording, on every platform, while the video recording (taken from the combined source) had it. This went unnoticed because the assistant's transcript comes from the voice model's own text output rather than from Amazon Transcribe, so transcripts looked complete. The recording is now written from the same audio that is sent to Transcribe, so a recording is by construction what was transcribed.

- **MicroVM: the audio pipeline was being built twice.** The MicroVM lifecycle retries the image's readiness hook, which re-ran the container's audio setup, and PulseAudio silently renames a duplicate device rather than refusing it — so a running Virtual Participant had six audio sinks and eight sources where it should have had three and four. The harmful duplicate mixed the assistant's voice into the transcription sink twice at two different latencies. Because the readiness hook runs at image build time, the duplicate pipeline was captured in the snapshot and inherited by every launch. The setup is now idempotent, and the microphone device the meeting uses is pinned explicitly instead of being left to PulseAudio to choose.

- **Fixed meeting joins failing with a timeout after the page had already loaded.** The browser was given only 20 seconds to load a meeting page and was made to wait for every last analytics and advertising element on it, which on a busy page never finishes. Joins now allow a realistic amount of time, proceed as soon as the meeting page itself is ready, and no longer abandon a page that arrived slightly late. All platforms were affected; Zoom surfaced it first because it is the only one with a sign-in step before the join.

## [0.3.6] - 2026-08-06

### Added

- **Desktop Capture App (native macOS and Windows) — a new meeting source** — A native menu-bar (macOS) / system-tray (Windows) app that streams your microphone **and** your computer's system (meeting) audio to LMA, so meetings joined from a **native desktop app** (Zoom, Teams, Webex, Slack, phone bridges, …) can be transcribed with **no browser tab and no Virtual Participant bot**. macOS captures via ScreenCaptureKit loopback + AVAudioEngine, Windows via WASAPI loopback + capture; both interleave into the same 2-channel PCM the browser sources send and feed the unchanged transcriber (mic → "My Mic"/AGENT, system → "Meeting Audio"/CALLER). Sign in with your normal LMA username/password. The UI offers start/stop/pause, mute mic/system, live level meters, an elapsed timer, recording status in the Dock/taskbar, start/stop notifications that deep-link to the meeting, recent meeting names, and a **Settings** window for editable speaker labels and a microphone picker; both also run headless as a CLI. Distributed from the web UI's **Sources ▸ Desktop Capture App** page as a preconfigured source zip per platform (the new `lma-desktop-capture-app-stack` runs a CodeBuild that bakes this deployment's endpoint, Cognito settings, stack name and version into `lma-config.json`; you build locally with `install-macos.sh` or `build-windows.ps1`, both of which support one-step uninstall). Each deployment's client installs as a **separate app** with its own settings, permission grants and login item, so multiple LMA stacks can coexist on one machine. Two capabilities ship with it:

  - **Optional screen video recording.** The app can also capture a chosen screen or window as H.264 video (macOS: ScreenCaptureKit + VideoToolbox; Windows: Windows.Graphics.Capture + Media Foundation) and stream it to the transcriber over a **second WebSocket**, so it never delays real-time audio or transcription. The server muxes it with the meeting audio into a single seekable MP4 in the recordings bucket and publishes it on the existing `ADD_S3_VIDEO_RECORDING_URL` path, so it plays back in the meeting detail page's **Recording Video** player — the same place Virtual Participant recordings appear. Capture runs at ~5 fps up to 1080p, is **off by default** and enabled per user in **Settings (⚙)**, and needs no new macOS permission (it reuses the Screen Recording grant system-audio capture already requires). Disable it deployment-wide with the existing `EnableVideoRecording` parameter. See [Optional: record screen video](docs/desktop-capture-app.md#optional-record-screen-video).
  - **Recording-consent gate.** These apps are LMA's most discreet capture method (no bot, no browser tab indicator), so both show a deployment-configurable disclaimer before the **first** recording on a machine, keep a standing reminder next to Start, and store an **auditable consent record** — the date plus the exact text agreed to — viewable in Settings; if the deployment changes the disclaimer text, the gate returns once. Set the text with the new `RecordingDisclaimer` parameter on the Desktop Capture App stack (default matches the browser extension's wording). See [Recording consent](docs/desktop-capture-app.md#recording-consent).

  See [Desktop Capture App](docs/desktop-capture-app.md) and the updated [Meeting Sources](docs/meeting-sources.md) comparison.

- **Microsoft Teams SDK join method for the Virtual Participant** — The VP can now join Teams meetings via [Azure Communication Services Teams interoperability](https://learn.microsoft.com/azure/communication-services/concepts/teams-interop) instead of automating the Teams web client, replacing brittle DOM scraping with deterministic SDK events (join/lobby/connected state, roster, dominant speaker, meeting chat, call-end codes). The live VNC view now shows a full meeting UI with video gallery, active-speaker highlighting and screen share, and audio capture/transcription, voice assistant and Simli avatar all work unchanged. Feature parity with the web-client method — intro message, chat commands (`LMA leave`/`stop`/`start`), speaker labels, avatar camera + watchdog, and the alone/host-ended/removed exit reasons — with **no Teams sign-in step and no CAPTCHA exposure**. ⚠️ Enablement mirrors the Zoom SDK pattern: set the new `AcsConnectionString` CloudFormation parameter and Teams joins via the SDK automatically (optional `MEETING_TEAMS_METHOD=auto|sdk|dom` override); leave it blank to keep the default web-client join. Requires only an Azure Communication Services resource (no Teams-side app registration or approval), and the meeting must be organized by a Microsoft 365 work/school account whose tenant allows anonymous join — personal `teams.live.com` meetings are not supported by ACS interop. Tenant-policy and meeting-type rejections are surfaced as actionable errors instead of ACS's opaque `code=490 subCode=4502`. See [Teams ACS SDK Join](docs/teams-acs-sdk.md).

### Changed

- **⚠️ Node 22 LTS is now required for building and developing LMA** — CI, the dev toolchain and the runtime moved to Node 22 (`^22.22.2 || >=24.15.0`) to unblock the UI test stack's jsdom 30, which hard-requires it; on Node 20 the entire UI suite failed to start with `webidl.util.markAsUncloneable is not a function`. Update your local Node before running `publish.sh` or the UI/transcriber builds. (#528)

- **Upgraded the web UI build toolchain to Vite 8 / Vitest 4** (`lma-ai-stack/source/ui`) — Vite 8's Rolldown + Oxc pipeline no longer honors the Vite 7 `esbuild` / `optimizeDeps.esbuildOptions` config the UI used to transform JSX inside plain `.js` files (the legacy CRA convention), so `vite.config.js` was migrated to an `enforce: 'pre'` plugin running `transformWithOxc`, `optimizeDeps.rolldownOptions`, and a function-form `manualChunks`. Two CommonJS-only dependencies that Rolldown resolves differently were also dealt with: `react-audio-player` was dropped for a native `<audio controls>` element, and `react-use-websocket` imports are now interop-agnostic. Production build and the Vitest suite both pass. (#315)

- **Dependency updates across every stack, with Dependabot now automating them** — Added `.github/dependabot.yml` (npm across the UI, transcriber, VP backend, browser-extension and tooling manifests; pip across the Lambda functions/layers; GitHub Actions) with weekly grouped minor/patch PRs against `develop`, then landed a large batch of those updates — including the UI, transcriber, VP backend, browser-extension, docs-site (Astro/Starlight) and the boto3/botocore family — resolving transitive ReDoS/DoS/prototype-pollution advisories with no build regressions. Two advisories are intentionally deferred because their fixes are breaking rather than patch-level: **fastify** (transcriber, 3.x→5.x major migration) and **urllib3** (a Lambda's `urllib3<2` pin is held for boto3 runtime compatibility; the cited advisory requires the 2.5.0 line and does not apply to this code's usage).

### Fixed

- **One bad client or call could kill the shared transcriber task and drop every concurrent meeting** — Three separate paths in the WebSocket transcriber took the whole Fargate task down with them: a Kinesis `put` was fire-and-forget, so a throughput exception became an unhandled rejection and process death; the message handler called `process.exit(1)` on a malformed frame or per-call I/O error; and there was no shutdown handling, so an ECS deploy, scale-in or health replacement `SIGKILL`ed the task mid-flight and lost end-of-call recording work silently. The Kinesis send is now awaited inside the surrounding try/catch, a per-socket error closes only that socket (with control-frame `JSON.parse` guarded separately), and a `SIGTERM`/`SIGINT` handler awaits in-flight end-of-call work with `StopTimeout: 120` so ECS allows the window.

- **Meeting recordings could silently overwrite each other** — The transcriber derived its S3 recording key from the call ID via a lossy filename sanitizer, so two differently titled meetings that normalized to the same string — or one meeting recorded twice — collided and the later recording replaced the earlier one with no error. Keys are now timestamped, matching the Virtual Participant's convention.

- **Transcription stopping or getting wrong timestamps during long meetings** — Amazon Transcribe streaming sessions end periodically on long calls, and both the WebSocket transcriber and the Virtual Participant scribe handled it incompletely: a graceful (non-error) server close ended transcription while audio kept flowing, and because Transcribe resets item `StartTime`/`EndTime` to 0 on each new request, post-reconnect segment and caption timestamps restarted from zero. Both now reconnect on clean closes as well as errors, carry a cumulative timeline offset so timestamps stay aligned with wall-clock across reconnects, count only *consecutive* failures toward the retry limit, and keep a meeting-long append-mode recording stream so a reconnect no longer truncates the recording. Hot-loop guards (a session must run ≥10s to count as healthy, plus a minimum backoff per reconnect) prevent a rapid close/reopen cycle from spinning, and the stale-session detector was tightened so an unrelated credential-expiry error no longer discards a still-valid `SessionId`. (#292, #465)

- **Teams Virtual Participant leaving meetings early on a transient roster misread** — The Teams VP could exit mid-conversation (recording stops hard, container exits with `EssentialContainerExited`/`exitCode=0`, VP row marked `COMPLETED`) whenever the Teams roster badge momentarily re-rendered, hid, or exposed an empty text node during a content-share transition, full-screen, or gallery/speaker re-layout — the old logic fired the graceful-leave path on the *first* reading that parsed to `<=1` attendees. Replaced with a debounced polling watchdog modeled on the one already proven on Zoom: it polls every 20s, requires **3 consecutive** genuine "`<=1` attendees" reads (~60s) before leaving, and treats a missing or empty badge as *unknown* rather than as "alone". (#317, #318, #399)
...
- **Zoom Meeting SDK join method for the Virtual Participant** — The VP can now join Zoom meetings via the [Zoom Meeting Web SDK](https://developers.zoom.us/docs/meeting-sdk/web/) (Client View) instead of automating the public Zoom web client. A localhost server hosts the SDK embed page (served from the bundled `@zoom/meetingsdk` assets with the COOP/COEP cross-origin-isolation headers the SDK requires), the existing CloakBrowser navigates to it, and the VP joins with a short-lived signature JWT — so the live VNC view, audio capture/transcription, voice assistant, and Simli avatar all work unchanged while replacing brittle DOM scraping with deterministic SDK events (join/leave, active speaker, chat, meeting status). Feature parity with the web-client method: intro message, chat commands (`LMA leave`/`stop`/`start`), speaker labels, avatar + camera watchdog, consent/AI-summary/interpretation popup dismissal, and the alone/host-ended/removed exit reasons. Enablement mirrors the Simli pattern — set the new `ZoomMeetingSdkClientId` / `ZoomMeetingSdkClientSecret` CloudFormation parameters and Zoom joins via the SDK automatically (optional `MEETING_ZOOM_METHOD=auto|sdk|dom` override); leave them blank to keep the default web-client join. When SDK credentials are configured the Create Virtual Participant modal hides the Zoom sign-in card (no Zoom account needed — the participant authenticates with a signature) and surfaces a `ZoomSdkEnabled` flag to the UI. Requires a Zoom **Meeting SDK** app (General App) with the programmatic-join use case enabled; joining meetings outside the app owner's account additionally requires Zoom to allowlist the app. See [Zoom Meeting SDK Join](docs/zoom-meeting-sdk.md).
