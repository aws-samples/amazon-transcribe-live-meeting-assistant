...
- **Voice assistant audio quality on Microsoft Teams.** The Virtual Participant's microphone is a virtual device carrying only the assistant's synthesised speech, but meeting clients ask the browser to run echo cancellation, noise suppression and automatic gain control over it by default. Applied to synthetic speech that has no room, echo or background noise, that processing gates and pumps the audio, which is audible as crackling. Those three stages are now switched off for the assistant's microphone, and the browser's audio settings are logged so this is diagnosable in future. A related timing bug is also fixed: the existing microphone override was installed too late to take effect on Teams at all.
