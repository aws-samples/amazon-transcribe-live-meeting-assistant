...
- **Speaker labels no longer hand a placeholder to a real person.** Every voice on the tab/meeting-audio channel is now numbered (`Speaker 1 (tab)`, `Speaker 2 (tab)`); only the microphone keeps the signed-in user's name. Previously the first voice heard on a channel inherited that channel's name, which for the tab is the placeholder "Other Participant" — so a meeting whose two remote speakers were separated *correctly* produced a transcript reading `Other Participant` beside `Speaker 1 (tab)`, and a reviewer reasonably concluded the two had been merged into a leftover bucket. Clustering that meeting's embeddings independently of the labels showed two voices 0.34 apart with the labels matching the clusters exactly: the diarization was right and the naming was misleading. Transcripts are read by people, exports and summarisation models alike, so a name that looks like a bucket is a correctness problem, not a cosmetic one.

- **Speaker turns inside one utterance are now split into separate transcript rows** — endpointing closes an utterance on silence, so two people speaking without a gap between them shared one row and one speaker label (the "one speaker per utterance" limitation). The engine now bakes **pyannote segmentation 3.0** (MIT, 6 MB ONNX, redistributed ungated by k2-fsa so no Hugging Face token is needed), runs it over each closed utterance, and emits one `final` per detected turn — each embedded and identified separately, and cut at a word boundary so no word lands in two rows. Three rules keep it from making transcripts worse: a change must persist 700 ms to count (a back-channel "mhm" does not split a sentence), analysis windows overlap by half with only the trusted middle of each used (a window seam is never mistaken for a turn), and a part too short to embed is merged back into its neighbour. Supporting this, the streaming recogniser now reconstructs word timings from sherpa's per-token output, and a segment without word timings is never split. Validated end to end against k2-fsa's own two-speaker sample: the turn is found at 7.89 s and the rows come back as two speakers with the words divided evenly. Off when `AsrSegmentationModelId=none` or when **Split rows on a speaker change** is unticked on the ASR Config page (no rebuild). The forward cut — cutting mid-utterance so live partials switch label immediately — is not in this change; splitting happens at segment close, where the whole utterance is available.
