...
- **Virtual Participant startup status copy now matches the hosting mode (Fargate vs EC2)** — When `VPLaunchType=FARGATE`, the VP startup statuses no longer describe EC2-only behavior. The `WAITING_FOR_CAPACITY` status previously always read *"Task is queued waiting for an EC2 host slot … the auto-scaler will launch a new host"* even on Fargate (where there are no host slots or auto-scaler), and `INITIALIZING` always said "Fargate" even on EC2. The Web UI now resolves launch-type-aware copy (the deployment's `VPLaunchType` is surfaced into the Web UI settings), falling back to launch-neutral wording when unknown, and the `get_virtual_participant_status` MCP tool uses launch-neutral wording.
- **Webex "Paste Meeting Invite" parsing of `j.php?MTID=…` launch links** — Pasting a Webex invite whose join link is a `https://SITE.webex.com/PATH/j.php?MTID=<token>` launch URL previously detected the platform as Webex but failed to extract a usable Meeting ID or password (the `MTID` is an opaque token, not a numeric meeting number, and the password is resolved server-side). The meeting-invitation parser now preserves the full launch URL as the Meeting ID for `j.php` links (extracting a password only from an explicit `password=`/`pwd=` query parameter), while still extracting the numeric ID from personal-room `…/meet/prNNN` URLs. The Virtual Participant Webex handler now navigates directly to a join URL when the Meeting ID is a full `http(s)://` link (mirroring how Teams meetup-join URLs are handled) instead of only typing a numeric meeting number into the join-by-number form.
- **Webex `j.php` join flow: app-download chooser + main-frame web client** — Joining via a `j.php` launch link lands on a "Join your Webex meeting" chooser page ("Download the Webex app" vs "Join from this browser") on a newer Webex build that loads the web client directly in the page (`preloader.html?…runInOwnPage=true`) instead of inside an iframe, and auto-raises Chromium's native *"Open xdg-open?"* external-protocol chooser to launch the desktop app. The Virtual Participant previously (a) clicked a stale `#broadcom-center-right` layout container that silently no-oped instead of the real "Join from this browser" button, and (b) waited only for the `#unified-webclient-iframe` / `thinIframe` iframes and hard-failed after 15s with *"Failed to access Webex meeting frame."* The Webex handler now clicks "Join from this browser" via robust text-based matching with an AI-resolver fallback, and detects the join UI in the main frame (new `mainframe` source) using `page.mainFrame()`. The native external-protocol dialog is browser chrome that doesn't block the page, so it's best-effort dismissed (CDP `Page.javascriptDialogOpening` handler + `page.on('dialog')`) rather than relied upon — the click is what joins. Profiles also keep `protocol_handler.excluded_schemes` for the meeting-app custom URL schemes, though cloakbrowser's patched Chromium binary may ignore that hint.
...
- **"Meeting URL" relabel + password hide for URL-based joins** — In the Create Virtual Participant modal, when the Meeting ID field contains a full `http(s)://` join URL the field relabels to **Meeting URL** and the separate **Meeting Password** field is hidden (a join URL carries its password server-side, so a separately-entered password is never submitted with one). A bare numeric ID keeps the existing **Meeting ID** + **Meeting Password (Optional)** layout.
- **Per-platform stored browser profiles** — The Virtual Participant's persisted Chromium profile is now keyed per user **and per meeting platform** (`profiles/{sha256(sub)}/{platform}/profile.tar.gz`) instead of a single shared profile across all platforms, so a Zoom-authenticated session is never reused for a Webex/Teams/Chime meeting. The Create Virtual Participant modal's **Stored browser profile** card now shows and removes the profile for the currently selected platform (the `getMyChromeProfileStatus`/`deleteMyChromeProfile` AppSync operations take an optional `platform` argument). Removing Zoom credentials now wipes only the Zoom profile rather than all platforms. Existing shared profiles from before this change are not migrated — the first join per platform after upgrade starts fresh.
