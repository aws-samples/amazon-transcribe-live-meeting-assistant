...
- **Persistent per-user Chromium profile in S3** — The VP container now hydrates a per-user `userDataDir` from a new `VPProfilesBucket` S3 bucket at launch and uploads the modified profile back to S3 at meeting end (sync-on-start / sync-on-end with the AWS SDK; Fargate doesn't grant FUSE so Mountpoint isn't used). Once a user has signed in once and solved any reCAPTCHA / 2FA via the live VNC viewer, Zoom plants a "trusted device" cookie in the profile that persists across meetings — subsequent VP launches reuse the session and skip both the reCAPTCHA *and* the bot-detection dialog entirely. Profile prefix is wiped automatically on credential removal. Concurrency-safe via an S3 conditional-PUT `lock.json` object with a 10-minute TTL so a crashed task can't permanently lock the profile. Bucket is KMS-encrypted, public access blocked, versioned.
...
- **Virtual Participant browser stack: `puppeteer-extra-plugin-stealth` + Chrome Stable on Debian Bookworm → `rebrowser-puppeteer` + Chromium on Alpine** — Migrated the VP container from `node:22-bookworm-slim` + `google-chrome-stable` to `node:22-alpine3.21` + apk-installed `chromium`, and replaced `puppeteer` + `puppeteer-extra` + `puppeteer-extra-plugin-stealth` with `rebrowser-puppeteer`. **Why:** `rebrowser-puppeteer` patches CDP at the runtime-binding layer, so pages can't detect the `Runtime.Evaluate` / `chrome.runtime.connect` shims that Stealth plugin had to add on top of vanilla Puppeteer — strictly fewer fingerprint signals than the Stealth-plugin arms-race posture. Alpine has materially fewer CVEs than bookworm-slim in ECR/Mirador scans (most of the bookworm findings were dynamically-linked glibc/openssl transitives that Alpine's musl/libressl variants avoid), and the image is ~300 MB smaller. Trusted-device cookies in the persistent S3 profile do most of the heavy lifting for Zoom bot-detection now, so the marginal benefit of Chrome-vs-Chromium is small enough that the CVE-noise reduction wins.
