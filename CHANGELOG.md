...
- **Dependabot configuration + transitive dependency patching** — Added `.github/dependabot.yml` (npm across the UI, transcriber, VP backend, browser-extension, and tooling manifests; pip across the Lambda functions/layers; GitHub Actions) with weekly grouped minor/patch update PRs against `develop`, so security bumps are auto-remediated instead of accumulating. Also refreshed the lockfiles for the internet-facing WebSocket transcriber app (`npm audit fix`: 12→3 advisories), the Virtual Participant backend (46→22), and the websocket-client utility (11→4), resolving transitive ReDoS/DoS/prototype-pollution advisories (micromatch, picomatch, brace-expansion, minimatch, js-yaml, fast-uri, and others) with no `package.json` / direct-dependency changes and no build regressions. Two advisories are intentionally deferred to follow-up work because their fixes are breaking, not patch-level: **fastify** (transcriber app, 3.x→5.x major migration) and **urllib3** (a Lambda's `urllib3<2` pin is held for boto3 runtime compatibility; the cited advisory requires the 2.5.0 major line and does not apply to this code's usage).

### Added

- **Microsoft Teams SDK join method for the Virtual Participant** — The VP can now join Teams meetings via [Azure Communication Services Teams interoperability](https://learn.microsoft.com/azure/communication-services/concepts/teams-interop) instead of automating the Teams web client. A localhost server hosts an embed page built from the [ACS UI Library](https://azure.github.io/communication-ui-library/) (`@azure/communication-react`'s `CallWithChatComposite`), the existing CloakBrowser navigates to it, and the VP joins as an anonymous external participant with a short-lived ACS user token minted in-container from the resource connection string — so the live VNC view (which now shows a full meeting UI with video gallery, active-speaker highlighting and screen share), audio capture/transcription, voice assistant, and Simli avatar all work unchanged while replacing brittle DOM scraping with deterministic SDK events and API calls (join/lobby/connected state, roster, dominant speaker, meeting chat, call-end codes). Feature parity with the web-client method: intro message, chat commands (`LMA leave`/`stop`/`start`), speaker labels, avatar camera + watchdog, and the alone/host-ended/removed exit reasons — with no Teams sign-in step and no CAPTCHA exposure. Enablement mirrors the Zoom SDK pattern — set the new `AcsConnectionString` CloudFormation parameter and Teams joins via the SDK automatically (optional `MEETING_TEAMS_METHOD=auto|sdk|dom` override); leave it blank to keep the default web-client join. Requires only an Azure Communication Services resource (no Teams-side app registration or approval), and the meeting must be organized by a Microsoft 365 work/school account whose tenant allows anonymous join — personal `teams.live.com` meetings are not supported by ACS interop. Tenant-policy and meeting-type rejections are surfaced as actionable errors rather than a generic "not admitted" — ACS reports every join refusal as the same opaque `code=490 subCode=4502`, so the embed page inspects the Teams conversation-proxy response and maps the real reason (tenant disables anonymous join, personal `teams.live.com` meeting, malformed meeting identifier). See [Teams ACS SDK Join](docs/teams-acs-sdk.md).

- **Audio Capture App (Native macOS) — a new meeting source** — A native macOS menu-bar app that streams your microphone **and** your computer's system (meeting) audio to LMA, so meetings joined from a **native desktop app** (Zoom, Teams, Webex, Slack, phone bridges, …) can be transcribed with **no browser tab and no Virtual Participant bot**. It captures system audio via ScreenCaptureKit loopback + the mic via AVAudioEngine, interleaves them into the same 2-channel PCM the browser sources send, and feeds the unchanged transcriber (mic → "My Mic"/AGENT, system → "Meeting Audio"/CALLER). Signs in with your normal LMA username/password via a dependency-free Cognito SRP implementation (validated against a reference impl offline). The menu-bar UI provides start/stop/pause, mute mic/system, live per-channel level meters, a red recording indicator, "Open in LMA", remember-email, and start-at-login; it also runs headless as a CLI (any `--flag`). Distributed from the web UI's new **Sources ▸ Audio Capture App (Native)** page as a preconfigured source zip (a new `lma-audio-capture-app-stack` runs a CodeBuild that bakes this deployment's endpoint + Cognito settings into `lma-config.json` and zips the source; the user builds locally with `install-macos.sh` — a macOS/ScreenCaptureKit app can't be cross-compiled by the Linux pipeline). macOS is available now; a Windows app is planned and iOS/Android are under consideration. See [Audio Capture App (Native)](docs/audio-capture-app.md) and the updated [Meeting Sources](docs/meeting-sources.md) comparison.
...
- **Voice assistant and avatar speech attributed to the last human speaker on the Teams SDK join path** — When the LMA voice assistant answered a question, the transcript labeled its reply with whoever spoke last (e.g. "Jeremy Feldman") instead of the LMA identity. Speaker attribution needs two signals combined: who the meeting platform reports as speaking, *and* whether the voice agent is currently speaking (`AgentSpeakingDetector`, measuring RMS on the `agent_output.monitor` source that carries both the assistant's TTS and the avatar's audio). The Teams SDK handler only had the first — ACS reports either the VP itself (filtered out as the bot's own name, leaving the previous human as the current speaker) or the last human, so the agent's turn never became the LMA identity. It now subscribes to the detector's `started`/`stopped` events the same way the Teams web-client path does: the agent's turn is attributed to the LMA identity and reverts to the last meeting speaker when it finishes, with the platform speaker no longer able to overwrite the label mid-utterance. This also restores translator-mode suppression of agent-origin transcript segments on this path, which keys off the same attribution.
- **Simli avatar invisible to other participants when a meeting SDK requests a fixed camera resolution** — The avatar rendered correctly in the VP's own local preview (and in the live VNC view) but every other attendee saw a blank video tile. The ACS calling SDK opens the camera with hard constraints (`width {min:1920,max:1920}`, `height {min:1080,max:1080}`), while the `getUserMedia` override handed back the avatar's 256×256 canvas-capture track unchanged. WebRTC negotiated a 1080p sender against a 256×256 source, encoded ~32 frames, then the sender wedged permanently — `outbound-rtp.framesSent` frozen while `media-source.frames` kept climbing, with `qualityLimitationReason: "none"` and ICE connected, so it read as neither a CPU nor a network fault. `buildSimliStream` now honors the requested geometry, drawing the avatar letterboxed into a canvas at the requested size and frame rate (via `setInterval`, so background throttling can't stall it) and falling back to the raw track if anything fails. Callers that don't request a specific size (the Zoom and Webex paths) are unaffected.
- **Teams SDK sessions lingering in the roster as "Leaving…" ghost participants** — Each VP run left an orphaned participant visible to everyone in the meeting, accumulating across runs. The leave path invoked the adapter's `leaveCall()` without awaiting it, so the browser closed before ACS finished disconnecting. The page-side leave is now async and polls until the call state reaches `Disconnected` (6s cap), and the Node side awaits it with an 8s timeout and logs the outcome.
- **Transcription reconnect correctness for long meetings (follow-up hardening for #292)** — Follows the initial #292 reconnect fix and corrects/hardens it consistently across the WebSocket transcriber (`transcribe.ts`) and the Virtual Participant scribe (`scribe.ts`). (1) **Timeline offset corrected** — the cumulative reconnect offset (Transcribe resets per-item `StartTime`/`EndTime` to 0 on every new request but they are monotonic *within* a session) now stays *constant* for the duration of each session and advances by the highest segment `EndTime` seen only when the session ends. A prior iteration advanced it on every result, which compounded the offset and pushed segment/caption timestamps far past wall-clock *even with zero reconnects*. (2) **Transcriber shared audio stream isolated** — each session now consumes the shared audio input via a per-session `PassThrough` sink (`unpipe` + `destroy` on teardown) instead of iterating the shared stream directly, which on reconnect destroyed it and threw `Cannot call write after a stream was destroyed` on the next client write. (3) **VP now reconnects on a clean server-side close too** (previously only its error path retried, so a graceful Transcribe close silently ended transcription while audio kept flowing), with *consecutive*-failure counting and a meeting-long **append-mode** recording stream so reconnects no longer truncate the recording. (4) **Hot-loop guards** — a session must run ≥10s (not merely emit one event) to count as healthy, and every reconnect waits a minimum backoff, so a rapid close/reopen cycle can't spin. (5) **Stale-session detector tightened** — it now requires the word "session", so an unrelated credential "has expired" error no longer discards a still-valid `SessionId`.
- **Teams Virtual Participant leaving meetings early on a transient roster misread** — The Teams VP could exit mid-conversation (recording stops hard, container exits cleanly with `EssentialContainerExited`/`exitCode=0`, VP row marked `COMPLETED`) whenever the Teams roster badge (`span[data-tid="toolbar-item-badge"]`) momentarily re-rendered, hid, or exposed an empty text node — during a content-share transition, full-screen, or gallery/speaker re-layout. The old logic reacted to every roster `MutationObserver` callback and fired the graceful-leave path on the *first* reading that parsed to `<=1`, so a single transient DOM misread ended a live meeting (GitHub #317, #318). Replaced the fragile single-read observer with a debounced polling watchdog modeled on the one already proven on Zoom: it polls every 20s, requires **3 consecutive** genuine "`<=1` attendees" reads (~60s) before leaving, and treats a missing/empty badge as *unknown* (never a leave signal on Teams, where the badge legitimately disappears) rather than as "alone".
...
- **`docs-site` build failed from a clean checkout** — `npm run build` (and `dev`/`start`) errored with `AstroUserError: The slug "…" does not exist` because the Starlight content collection under `src/content/docs/` is populated by symlinks that `setup.sh` creates from the top-level `docs/*.md`, and that step was never wired into the npm scripts — so a fresh clone (or CI) built against an empty collection while the sidebar referenced real slugs. Added `setup` / `prebuild` / `predev` / `prestart` npm lifecycle scripts so `setup.sh` runs automatically before every build and dev start; `npm run build` now succeeds standalone (44 pages). Surfaced during the Starlight 0.41 upgrade, which validates sidebar slugs strictly (older versions silently ignored missing entries).
- **Audio Capture App: Gatekeeper "cannot verify … is free of malware" popup on install** — `install-macos.sh`'s quarantine-clearing step was gated on `xattr -r -p`, which exits non-zero whenever any directory in the tree lacks the attribute — the normal case when the download is unzipped with the CLI `unzip` (only files get tagged). The guard therefore skipped the `xattr -dr` clear, so launching the built app still tripped Gatekeeper. The clear now runs unconditionally.
- **Audio Capture App: `make-app.sh` left the app unsigned** — `lma-config.json` was copied into `Contents/MacOS/` next to the executable, which `codesign` rejects ("code object is not signed at all"), aborting the `set -e` build and leaving the bundle unsigned (compounding launch failures on Apple Silicon). The config now goes in `Contents/Resources/` (where `Config.swift` already looks), the assembled bundle has quarantine stripped before signing, and code-signing succeeds.

### Changed (docs/UI)

- **Audio Capture App platform roadmap wording** — the platform list now distinguishes a **planned** Windows app ("Coming soon") from **iPhone/iPad and Android**, which are marked **"Under consideration"** rather than implying they're committed/imminent. Updated in the UI page and the docs.
