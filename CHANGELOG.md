...
- **Transcription stopping partway through long meetings (browser Stream Audio / extension path *and* Virtual Participant)** — Amazon Transcribe closes its streaming result stream server-side on its own schedule (idle behaviour, session duration limits, transient server close). The WebSocket transcriber only restarted the session from its `catch` block, so when the result stream ended *without throwing* the `for await` loop simply completed, the function returned, and transcription stopped **permanently** while the client kept streaming audio into a dead pipe (GitHub #292). A second defect compounded it: `retryCount` was never reset after a healthy session, so ~5 transient reconnects spread across a long meeting cumulatively hit `MAX_RETRIES` and aborted with only a server-side log. `startTranscribeSession` now (a) reconnects on an unexpected clean stream close unless the call has legitimately ended, (b) counts only *consecutive* failures toward `MAX_RETRIES` — resetting once a session produced a transcript event or ran ≥10s, and (c) applies capped linear backoff between failed restarts. In addition, a reused `SessionId` that Transcribe has closed server-side (expired / not found / invalid) is now detected and dropped so the next attempt starts a **fresh** session instead of failing every retry on the stale id. Because Transcribe's per-item `StartTime`/`EndTime` reset to 0 on every (re)connect (reusing `SessionId` does *not* resume the timeline), both the transcriber and the Virtual Participant now carry a **cumulative time offset** across reconnects — advanced by the highest segment `EndTime` seen so far — so transcript segments continue the meeting timeline instead of jumping back to 0 and overlapping earlier segments. The same reconnect/offset/stale-session logic is applied consistently in the WebSocket transcriber (`transcribe.ts`) and the Virtual Participant scribe (`scribe.ts` / `kinesis-stream.ts`). (The related "speaker always shows n/a" symptom in #292 is a separate speaker-attribution/channel-mapping issue and is tracked separately.)
