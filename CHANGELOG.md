...
- **Webex `j.php` join flow: main-frame web client + native app-launch dialog** — Joining via a `j.php` launch link lands on a newer Webex build that loads the web client directly in the page (`preloader.html?…runInOwnPage=true`) instead of inside an iframe, and auto-raises Chromium's native *"Open xdg-open?"* external-protocol chooser to try to launch the desktop app. The Virtual Participant previously waited only for the `#unified-webclient-iframe` / `thinIframe` iframes and hard-failed after 15s with *"Failed to access Webex meeting frame."* The Webex handler now also detects the join UI in the main frame (new `mainframe` source) and uses `page.mainFrame()` for it; the per-user Chromium profile now marks the meeting-app custom URL schemes (`webex`, `msteams`, `zoommtg`, …) as `protocol_handler.excluded_schemes` so Chromium silently ignores the desktop-app handoff instead of prompting, with a `page.on('dialog')` auto-dismiss as a runtime safety net.
