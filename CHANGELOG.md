...
- **The documentation no longer lists Google Meet as a Virtual Participant platform, and a pasted Meet invitation now says so.** The README, the quick-start guide and the Virtual Participant page all listed Google Meet alongside Zoom, Teams, Chime and WebEx, but the Virtual Participant has no Google Meet handler and fails immediately with "Unsupported meeting platform". Meet meetings are captured with the [Chrome extension](docs/browser-extension.md) instead, which is what those pages were describing. The meeting-invitation parser also auto-filled the create form from a Meet invitation, so the failure arrived only after the user had reviewed and submitted every other field; it now rejects the invitation up front with a message pointing at the extension. Adding Virtual Participant support for Google Meet is tracked in [#661](https://github.com/aws-samples/amazon-transcribe-live-meeting-assistant/issues/661).
