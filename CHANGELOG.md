...
- **Virtual Participant browser stack: Puppeteer → Playwright, on stock Chromium** — The VP container migrated from `puppeteer` + `puppeteer-extra-plugin-stealth` to Playwright (`playwright-core`) driving the Debian `chromium` package via `chromium.launchPersistentContext`. **Why Playwright:** the platform handlers were rewritten against Playwright's `Page`/`ElementHandle` API for a more robust join flow (time-bounded CDP calls, per-step verification, AI DOM-resolver fallback). **Why stock Chromium:** an interim move to CloakBrowser (a patched anti-fingerprint Chromium) was evaluated and reverted — its WebRTC patch broke the Simli avatar's same-browser loopback video bridge (0 ICE candidates → black camera), suppressed CDP console forwarding, and wedged the renderer under load, while reCAPTCHA still triggered regardless. Stock Chromium (the VP's original browser) restores reliable WebRTC, console capture, and renderer behaviour. The image is `python:3.12-slim` + `apt install chromium`; Playwright launches it via `executablePath=/usr/bin/chromium` (`CHROME_BIN`), so no Playwright browser download is needed. Fresh profiles run a 7-site warmup (Google → HN → Wikipedia → zoom.us / app.chime.aws / teams.microsoft.com / web.webex.com) before the meeting URL so a first-time profile arrives with normal browsing history rather than zero state. Together with the 3p-cookie pre-write fix (Chrome v123+ default-blocks 3p cookies, breaking Zoom's cross-domain auth on a fresh profile) and the persistent S3 profile, fresh-profile Zoom joins from datacenter IPs are reliable.

- **Bigger image, smaller surface: `simli-client` v3 vendored into the image via esbuild** — The Simli avatar page now imports the `simli-client` ES module bundle from a fetch-intercepted in-image path (`http://local.simli/simli-client.bundle.mjs`) instead of loading it from a CDN at meeting time. The Dockerfile runs `esbuild` at build time to produce `dist/simli-client.bundle.mjs`, and `simli-avatar.ts` registers a Playwright route handler that serves it. Pinned local version is safer than the previous CDN dependency.
