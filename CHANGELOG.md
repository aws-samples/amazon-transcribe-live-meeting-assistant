...
- **Voice assistant audio no longer crackles on Microsoft Teams.** Teams (uniquely) requests an exact 1920x1080 video size, which put the avatar on a rescale path that redrew a full-resolution canvas 15 times a second. On a shared 2-vCPU host that starved the audio pipeline. The redraw now only repaints what changes, and the audio buffers were widened from 20ms to 80ms — still well below conversational latency.
