...
- **Translator Mode for Nova Sonic voice assistant** — New `meetingMode = translator` turns the LMA virtual participant into a real-time bidirectional AI interpreter. When enabled, the voice assistant joins a Zoom, Teams, Chime, or Webex call, listens to both parties, and speaks each utterance back in the other language — no human interpreter required. Powered by Amazon Nova Sonic polyglot voices. Configure the language pair (`translatorLanguageA` / `translatorLanguageB`) from the **Nova Sonic Configuration** page in the LMA Web UI; changes take effect on the next VP task start without a stack update. Also ships **Group Meeting Mode** (`meetingMode = group`) as the formal replacement for the legacy `groupMeetingMode` boolean (backward-compatible). Both modes require `VoiceAssistantActivationMode = always_active`. The Nova Sonic Configuration UI now shows a **Meeting Mode** selector (Normal / Group Meeting / Translator) with Translator Language A/B inputs, and displays an info alert when the deployment's activation mode is not `always_active`. ElevenLabs provider updated with equivalent mute-gate parity. In Translator Mode the scribe also suppresses Kinesis transcript segments whose current speaker label matches the VP’s own identity (`lmaIdentity` / `scribeIdentity`) OR that are produced while the agent is speaking (detected via `AgentSpeakingDetector`) so Amazon Transcribe's mis-transcription of the agent's foreign-language TTS never reaches the transcript panel — users read translations via the UI's built-in transcript translator instead. See [Voice Assistant](docs/voice-assistant.md) and [Nova Sonic Setup](docs/nova-sonic-setup.md).

- **AI Translator Mode demo page** — New self-contained [`docs/embeddable-translator-demo.html`](docs/embeddable-translator-demo.html) demonstrates the Translator Mode end-to-end inside an AWS-branded demo page. Includes a language pair selector (31 languages), meeting form, and a 2x2 monitor grid of LMA iframes (live VNC view, meeting details, live transcript, AI summary) driven via `postMessage`. See [Embeddable Components](docs/embeddable-components.md).

- **Custom date-range for the meeting list + scalable GSI-backed query** — The meeting list is now scoped by an explicit start/end datetime rather than client-side shard fan-out. A new `TypeDateIndex` GSI on `EventSourcingTable` (keyed on `ItemType` + `SK`, with an `INCLUDE` projection of the visible list fields) is populated by a `createCall` VTL stamp on new `cls#…` list rows and a CloudFormation Custom Resource + Step Functions Distributed Map backfill (`backfill_gsi_attributes`) that idempotently stamps pre-existing rows. Two new AppSync Lambda-backed queries replace the legacy `listCalls` / `listCallsDateShard` / `listCallsDateHour` resolvers: **`listCallsDateRange(startDateTime, endDateTime, limit, nextToken)`** runs a single GSI query (`SK BETWEEN ts#<start>..ts#<end>#~`), enforces Owner/`SharedWith` RBAC server-side from the GSI projection, then `BatchGetItem`s the `c#<CallId>` detail rows so the client receives fully-hydrated `Call` objects in one round-trip; **`getCallCount(startDateTime, endDateTime): { count, truncated }`** uses a cheap admin fast path (`Select="COUNT"`) and an RBAC-filtered non-admin path (both bounded) with a `truncated` flag so the UI can display `N+`. UI adds a **Custom…** option in the Load dropdown that opens an IDP-style modal with explicit Start date + Start time (UTC) + End date + End time (UTC) inputs (±365-day window validation), relative presets at 2h / 4h / 8h / 1d / 2d / 1w / 2w / 30d, and persists the chosen range to `localStorage`. The list header now shows the RBAC-filtered server-side total (`(42 loaded of 127)` / `(500+)`). `use-calls-graphql-api` was rewritten to cursor-paginate a single `listCallsDateRange` query and uses a `dateRangeRef` useRef so `onCreateCall` / `onUpdateCall` / `onShareMeetings` subscription handlers stay stale-closure-safe when the user changes the range mid-paginate. The MCP `list_meetings` tool also switched to the same GSI path (dead `scan_recent_meetings` removed) so every meeting-listing path avoids DynamoDB scans and scales to tens of thousands of meetings and 1000+ concurrent users. See [Web UI Guide](docs/web-ui-guide.md).
...
- **Simli avatar "track needs re-connection" log spam every second with frequent mid-meeting reconnects** — the Simli reconnect poll in `index.ts` fired every 1s and tore down the WebRTC bridge whenever it couldn'''t see `window.__simliCurrentTrack` in the live state. Every deliberate reconnect itself closes the old RTCPeerConnection, which ends the old track, which synchronously fires `track.onended` and nulls `__simliCurrentTrack`; the next 1s poll then sees `null` and schedules ANOTHER reconnect before the new track from the in-flight offer/answer exchange has arrived (~700-1000ms). Result: a self-sustaining reconnect loop that produced a wave of "Simli video track needs re-connection...", "Connecting Simli video stream to meeting page...", "Simli track not ready - waiting up to 2s for reconnection" every second for the whole meeting, plus occasional cases where the avatar never stably connected to the meeting page. Two complementary fixes:

  - **On-demand reconnect, no polling timer** — the 1s background reconnect poll was fundamentally fighting against Zoom's own media-pipeline lifecycle: every deliberate reconnect closes the old `RTCPeerConnection`, which ends the old track, which synchronously fires `track.onended` and nulls `__simliCurrentTrack`; the poll then sees null, waits 3s, and reconnects again — producing a fresh track that arrives `muted=true` with `width/height=undefined` until RTP starts flowing, by which point Zoom has already shown "no camera" in the participant grid. Replaced the timer with on-demand reconnect via puppeteer's `page.exposeFunction('__simliRequestReconnect', ...)`: the meeting-page's `getUserMedia` override calls `window.__simliRequestReconnect()` **inline** whenever the track is missing, ended, or muted, waits up to 8s (50ms poll) for `__setSimliVideoTrack` to land a fresh live+unmuted track, and only then returns. This handles all three Zoom page transitions (join page → lobby → waiting room → in-meeting) cleanly because each new document re-runs `evaluateOnNewDocument` and the override requests a reconnect the first time Zoom calls `getUserMedia` on the new page.
  - **track.clone() on return** — every `getUserMedia` call now returns `simliVideoTrack.clone()` instead of the source track. The clone shares the same underlying Simli source, so when Zoom's video pipeline calls `.stop()` on its handle (it does, during in-meeting transition), only the clone ends — our original track stays live and subsequent `getUserMedia` calls on the new Zoom document return a fresh clone from the same long-lived source with no reconnect needed.
  - **Liveness check includes muted** — `isSimliTrackLive()` now requires `track.muted === false` in addition to `readyState === 'live'`, so the override never hands a muted track to Zoom (which would cause it to show "camera off" until RTP started flowing).
  - **getUserMedia override always returns the Simli track** — the meeting-page `getUserMedia` override previously checked a `simliStreamReady` boolean that could get out of sync during reconnects, and would fall through to the real `getUserMedia` (which returns an empty camera inside the ECS container) whenever a 2s wait timed out. It also respected Zoom's `advanced` size-constraint probes (e.g. `width/height.ideal: 320x180` with `advanced: [min:640x360, max:640x360]` against a 512x512 Simli source) and treated the resulting mismatch as "track not ready". The override now gates purely on `simliVideoTrack.readyState === 'live'` (not the stale boolean), always returns the Simli track regardless of the requested dimensions (WebRTC is lenient on consumer-side frame sizes), and extends the reconnection wait to 5s at 50ms granularity so a Zoom page transition mid-reconnect no longer exhausts the budget and causes the avatar to silently drop from the participant grid.

- **Virtual Participant failed to join meeting when `TranscribeLanguageCode` is set to `identify-language` or `identify-multiple-languages`** — the VP scribe was passing the sentinel string straight into Amazon Transcribe streaming'''s `LanguageCode` field, which only accepts real ISO codes (`en-US`, `hi-IN`, `es-ES`, etc.), so every new session was rejected with a validation error and `ERR_STREAM_PREMATURE_CLOSE` escaped from the retry loop into `emergencyCleanup`. The scribe now maps the two sentinels to the correct Transcribe streaming SDK shape — `IdentifyLanguage: true` or `IdentifyMultipleLanguages: true`, plus `LanguageOptions` (from the new `TRANSCRIBE_LANGUAGE_OPTIONS` env var) and optional `PreferredLanguage` (from `TRANSCRIBE_PREFERRED_LANGUAGE`) — and omits `LanguageCode`. As a second defense, the `startTranscription()` retry loop now short-circuits on non-retryable errors (`BadRequestException`, `ValidationException`, `InvalidParameterException`, any `httpStatusCode === 400`, or the Transcribe `non-retryable streaming request` string) instead of burning 5×5s retries while FFmpeg teardown races the next attempt. `TRANSCRIBE_LANGUAGE_OPTIONS` is now normalized (whitespace stripped, empty entries dropped) before being sent to Transcribe, matching the service’s `^[a-zA-Z-,]+` regex constraint on `LanguageOptions`. The `audioStream` async generator now swallows `ERR_STREAM_PREMATURE_CLOSE` when it fires during a deliberate transcription teardown, so an after-the-fact stream-close event no longer bubbles up as an `uncaughtException` that triggers `emergencyCleanup`. Removed a bug in the inner `handleTranscriptEvents` error classifier that was incorrectly treating `'''non-retryable streaming request'''` as retryable.
...
- **Web chat assistant failed to retrieve the transcript for meetings whose CallId contains `+`** (e.g. "Accelerator Advice - IDP + RCA-2026-05-05"). The chat UI is an iframe loaded from `/strands-chat.html?callId=...`, and the CallId was being interpolated into the query string without URL-encoding. Inside the iframe, `URLSearchParams.get("callId")` decodes a literal `+` as a space (per `application/x-www-form-urlencoded` rules), so the Strands agent received a corrupted CallId and its transcript tool found no matching call — while the end-of-call summary still worked because it is injected into the prompt from the parent page. Both chat iframe `src` assignments (`CallPanel.jsx` and `EmbedCallDetails.jsx`) now wrap `item.callId` with `encodeURIComponent`, restoring transcript retrieval for meeting ids containing `+` (and any other URL-reserved characters).
