...
- **`EnableDataRetentionOnDelete` now covers every stateful resource in the deployment.** It previously governed some of them, which meant a stack deletion could remove data the setting implied it would keep — including prompt templates, chat button definitions, voice assistant settings and ASR calibration authored from the web UI, the Cognito identity pool, and the knowledge base's vector index. All of these now follow the parameter, and [Stack Updates & Upgrades](docs/stack-updates-and-upgrades.md#data-retention-on-stack-deletion) lists exactly what is retained. ⚠️ Two notes for upgraders: deletion policies are read from the deployed template, so the new behaviour applies only after this update completes; and retained resources keep incurring storage charges until you remove them by hand.
