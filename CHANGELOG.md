...
- **Optional screen video capture from the Desktop Capture App** — The desktop clients can now also capture a chosen screen or window as H.264 video (macOS: ScreenCaptureKit + VideoToolbox; Windows: Windows.Graphics.Capture + Media Foundation) and stream it to the transcriber over a **second WebSocket** so it never delays real-time audio or transcription. The server muxes it with the meeting audio into a single seekable MP4 in the recordings bucket and publishes it to the existing `ADD_S3_VIDEO_RECORDING_URL` path, so it plays back in the meeting detail page's **Recording Video** player — the same place Virtual Participant recordings appear. Capture runs at ~5 fps up to 1080p, is **off by default** and enabled per user in **Settings (⚙)**, and needs no new macOS permission (it reuses the Screen Recording grant system-audio capture already requires). Can be disabled deployment-wide with the existing `EnableVideoRecording` parameter. The stream is purely additive: older clients and servers are unaffected. See [Desktop Capture App → Optional: record screen video](docs/desktop-capture-app.md#optional-record-screen-video).

- **Recording-consent gate for the Desktop Capture App** — The native apps were the only LMA capture surface without a recording-consent warning, and they are the most discreet (no bot, no browser tab indicator). Both now show a deployment-configurable disclaimer before the **first** recording on a machine, keep a standing reminder next to Start, and store an **auditable consent record** (date plus the exact text agreed to) that is viewable in Settings; if the deployment changes the disclaimer text, the gate returns once. The text is set with the new `RecordingDisclaimer` parameter on the Desktop Capture App stack (default matches the browser extension's wording). See [Desktop Capture App → Recording consent](docs/desktop-capture-app.md#recording-consent).
...
- **Virtual Participant EC2 hosts are now patched by SSM Patch Manager, and transcriber images always rebuild from a fresh base** — The VP host UserData asserted that Patch Manager owned OS patching, but no baseline, maintenance window or association had ever been defined, so hosts drifted out of patch compliance. A patch baseline plus a **weekly off-peak maintenance window** (`AWS-RunPatchBaseline`, Install + RebootIfNeeded) now targets the VP Auto Scaling group, keeping reboots out of meeting hours rather than at boot. Separately, the transcriber's CodeBuild `docker build` now passes `--pull --no-cache`, so a cached base or apt layer can no longer re-bake stale (unpatched) OS packages into the image. (#327)
...
- **`docs-site` build failed from a clean checkout** — `npm run build` (and `dev`/`start`) errored with `AstroUserError: The slug "…" does not exist`, because the Starlight content collection is populated by symlinks that `setup.sh` creates from the top-level `docs/*.md` and that step was never wired into the npm scripts — so a fresh clone or CI built against an empty collection while the sidebar referenced real slugs. Added `setup` / `prebuild` / `predev` / `prestart` lifecycle scripts so `setup.sh` runs automatically; `npm run build` now succeeds standalone. (#331)
