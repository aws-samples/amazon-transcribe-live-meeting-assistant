...
- **Persistent per-user Chromium profile in S3** — The VP container now hydrates a per-user `userDataDir` from a new `VPProfilesBucket` S3 bucket at launch and uploads the modified profile back to S3 at meeting end. The whole `userDataDir` round-trips as a single `profile.tar.gz` keyed by `sha256(cognitoSub)` (one profile per user — all meeting platforms share state, so an SSO sign-in to office.com benefits a later Teams join). Once a user has signed in once and solved any reCAPTCHA / 2FA via the live VNC viewer, Zoom plants a "trusted device" cookie in the profile that persists across meetings — subsequent VP launches reuse the session and skip both the reCAPTCHA *and* the bot-detection dialog entirely. Last-write-wins on concurrent VPs for the same user (the loser just costs themselves session cookies and re-challenges next launch). Bucket is KMS-encrypted, public access blocked, versioned.

- **ECS capacity provider auto-scaling for the VP cluster** — The VP cluster now uses an `AWS::ECS::CapacityProvider` with `ManagedScaling` (TargetCapacity=100, step 1-2, instance warmup 90s) wired as the cluster's default strategy via a new `ClusterCapacityProviderAssociations` resource. RunTask drives `CapacityProviderStrategy` (instead of `LaunchType=EC2`), so when the existing hosts are full ECS automatically launches new hosts up to `VPMaxInstances` to absorb the queued task. New `WAITING_FOR_CAPACITY` VP status is written when a task is queued waiting for a host to come up — the UI shows a friendly "Waiting for compute capacity… auto-scaler is launching a new host (typically 60-90s)" message instead of a silent `INITIALIZING`. The EC2 container memory cap was reduced from 3500 MB to 2500 MB (1.85× headroom over the observed peak of ~1348 MB) so the default `t3.medium` (1 VP/host) and bigger instances (e.g. `t3.large` = 3 VPs/host) all share the same per-VP footprint and the auto-scaler does the right thing at any size.

- **AI sign-in hardening: existing-session fast-path, stuck-loop detector, cheap manual-action poll** — The AI-driven Zoom sign-in loop now has three new behaviors that materially reduce both Bedrock cost and bot-detection trips. **Existing-session fast-path**: before calling the full sign-in, probe `zoom.us/myhome` and check for the auth cookie — if the persistent S3 profile already has a valid session, the entire AI loop is skipped. **Stuck-loop detector**: if Claude returns the same action signature (kind + selector) twice in a row AND the page hasn't changed (URL + visible-element fingerprint), bail to `MANUAL_ACTION_REQUIRED` instead of burning the iteration budget on a dead-end (the previous behaviour was 17+ no-progress retries when invisible reCAPTCHA was rejecting the form). **Cheap manual-action poll**: after escalating to `needs_human` the loop switches from "screenshot + Bedrock every 3 sec" to a free DOM-only auth-cookie poll every 5 sec for the rest of the timeout — no Bedrock spam while waiting for the human. Together these reduce typical sign-in cost from 4-20 Bedrock calls to 0 (returning user) or 4-8 (first sign-in) and prevent the runaway-loop pattern that was tripping reCAPTCHA scoring.

- **Granular Virtual Participant startup statuses** — The VP now reports a fine-grained status as it boots: `BOOTING` (pushed from `entrypoint.sh` before Node even starts), `REGISTERING_NETWORK`, `HYDRATING_PROFILE`, `LAUNCHING_BROWSER`, `VNC_READY`, `CONNECTING`, `JOINING`, `MANUAL_ACTION_REQUIRED`, `ACTIVE`. The UI exposes each as a distinct status badge with a human-readable summary, replacing the previous behaviour where the VP would sit at `INITIALIZING` for 60-90 seconds with no indication of progress. The `get_virtual_participant_status` MCP tool verbalizes each state for AI agents. See [Virtual Participant → Status Lifecycle](docs/virtual-participant.md#status-lifecycle).

- **`MANUAL_ACTION_REQUIRED` UI alerts** — When a VP hits a CAPTCHA, 2FA prompt, SSO redirect, unknown consent dialog, or Zoom bot-detection challenge that needs human input, the LMA UI now surfaces it three ways: a persistent **Flashbar alert** at the top of every page (with a direct link to the affected meeting), an **action banner** above the live VNC viewer on the VP detail page, and (with permission) a **browser notification + audio chime** so users can be tabbed-away or in another window. Alerts are dismissible per-VP with `localStorage`-backed memory so they don't reappear on every refresh once handled. Backed by a one-time backfill query on page load so anything fired while the tab was closed still shows up. See [Web UI Guide → Manual Action Alerts](docs/web-ui-guide.md#manual-action-alerts).
...
- **Virtual Participant browser stack: `puppeteer-extra-plugin-stealth` + Chrome Stable on Debian Bookworm → CloakBrowser stealth Chromium on Debian slim** — Migrated the VP container from `node:22-bookworm-slim` + `google-chrome-stable` + `puppeteer-extra-plugin-stealth` to `python:3.12-slim` (glibc) + [CloakBrowser](https://github.com/CloakHQ/cloakbrowser) 0.3.29 (Chromium 146.0.7680.177.3). **Why:** CloakBrowser ships a Chromium binary with 32+ source-level C++ patches (canvas / WebGL / audio / fonts / GPU / WebRTC / TLS / automation signals), so detection sites see a real browser because it *is* a real browser — strictly fewer fingerprint signals than any JS-injection or CDP-runtime-patch approach. The cloakbrowser package is `pip install`-ed and the binary is pre-downloaded at build time (`python -m cloakbrowser install`); the JS app uses `cloakbrowser/puppeteer`'s `launchPersistentContext`. Each user gets a stable per-Cognito-sub fingerprint seed (`fingerprintSeedForUser()`) so they look like the same returning visitor across launches. Fresh profiles run a 7-site warmup (Google → HN → Wikipedia → zoom.us / app.chime.aws / teams.microsoft.com / web.webex.com) before the meeting URL so they look like a real user with browsing history, not a fresh datacenter Chromium. The base image is glibc (cloakbrowser's binary is glibc-only — Alpine/musl was ruled out) but trades to a smaller final image than bookworm-slim once Chrome Stable is gone. Together with the 3p-cookie pre-write fix (Chrome v123+ default-blocks 3p cookies, breaking Zoom's cross-domain auth on a fresh profile) and the persistent S3 profile, fresh-profile Zoom joins from datacenter IPs now pass bot-detection.

- **Bigger image, smaller surface: `simli-client` v3 vendored into the image via esbuild** — The Simli avatar page now imports the `simli-client` ES module bundle from a fetch-intercepted in-image path (`http://local.simli/simli-client.bundle.mjs`) instead of loading it from a CDN at meeting time. The Dockerfile runs `esbuild` at build time to produce `dist/simli-client.bundle.mjs`, and `simli-avatar.ts` registers a Puppeteer request interceptor that serves it. Pinned local version is safer than the previous CDN dependency.

- **Ghost-cursor for human-like mouse pathing on the Sign-In / Join / Skip clicks** — `humanClick(cursor, page, target)` helper uses `ghost-cursor` (Bezier-curve mouse movement) for the cursor-pathing-sensitive clicks (Zoom Join, Sign-in, "Skip for now" interstitials, in-meeting chat-panel button). Audio/video SVG toggles still use the pre-existing `clickClickableAncestor()` helper since SVG elements can't be the cursor target directly. Reduces a bot-detection signal that simple `page.click()` misses.
