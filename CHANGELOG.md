...
- **A long meeting's summary, knowledge-base export and assistant answers now see the whole transcript.** The Lambda that assembles a meeting transcript read only the first page of results from DynamoDB. A query returns at most 1 MB of items *read*, and the filter that excludes in-progress (partial) segments is applied after that limit — so partial segments consumed the budget without appearing in the result, and even a moderately long meeting spanned several pages. Everything downstream received a transcript that stopped part-way through with nothing to indicate it was incomplete: the automatic summary, the transcript written to the knowledge base, and every Meeting Assistant answer described a partial meeting as though it were the whole one. Short meetings, which fit in one page, were unaffected — which is why this was invisible in testing. All pages are now read, bounded by a page limit and a check that the continuation key is advancing so that an unexpected response cannot make the function run until its timeout. No parameters change and no action is needed beyond the usual stack update; existing meetings are reassembled from stored segments, so a summary regenerated after the update will be complete.

- **A transcript segment timestamped at zero seconds is no longer dropped.** Producers that put `StartTime`/`EndTime` (or `BeginOffsetMillis`/`EndOffsetMillis`) directly on the Kinesis record — the on-demand MicroVM ASR engine, and any custom producer following the same shape — had those values tested for truth rather than for presence while being normalised, so a timestamp of exactly `0` was indistinguishable from an absent one and was replaced with nothing. Both fields are non-nullable in the GraphQL schema, so the segment was rejected when it was written and the line never reached the transcript. The most likely casualty was the opening segment of a meeting, which is the one that starts at zero. Timestamps are now checked for presence, so a zero is kept; `StartTime`/`EndTime` continue to take precedence over the millisecond offsets when a producer sends both. Meetings transcribed by Amazon Transcribe streaming or Transcribe Call Analytics were never affected, and no redeployment step is needed beyond the usual stack update.

- **A Teams Virtual Participant no longer leaves a meeting that is still in progress.** Teams removes the roster button — and with it the participant-count badge the VP watches — whenever the meeting toolbar collapses, most often during a content share or in a full-screen layout. That reading shared a threshold of roughly 40 seconds with the "everyone has left" check, so a routine screen share could end the VP's session mid-meeting; the exit was graceful, so the transcript and recording were saved and nothing appeared to have failed, leaving only a meeting that stopped recording part-way through. An unreadable count is now checked against the rest of the meeting interface and, while that is still present, does not count towards leaving — for up to half an hour at a stretch, which covers a long screen share, and is itself bounded because Teams sometimes leaves those controls on screen after a meeting has ended. A meeting showing neither a readable count nor any in-meeting controls is treated as over after about five minutes; the "everyone has left" check is unchanged at about 60 seconds. A meeting whose end Teams announces on screen is still detected on the first poll, as before. Where Teams shows no such screen, the VP now leaves up to about four minutes later than it used to — and, in the case where Teams also leaves its meeting controls on screen, up to about 35 minutes later (never more than an hour, whatever the new parameters are set to) — keeping recording until it does. Three new parameters — **`VPAttendeePollMs`**, **`VPPollsBeforeEnd`** and **`VPPollsBeforeEndMissing`** — expose the cadence and both thresholds for the Teams browser join path, for deployments that need different timing; defaults are unchanged behaviour and no deployment should normally need them. See [How the VP decides a meeting has ended](docs/virtual-participant.md#how-the-vp-decides-a-meeting-has-ended). ([#660](https://github.com/aws-samples/amazon-transcribe-live-meeting-assistant/issues/660))

- **A Virtual Participant can join Webex meetings again.** Webex replaced the pre-join display-name field with a custom element whose input sits inside a shadow root, so the Virtual Participant waited for a field that no longer existed and the join failed with a selector timeout. Both the current and the previous markup are now recognized. The name is also replaced rather than appended, so a name persisted from an earlier session no longer produces a doubled display name such as `LMALMA (user@example.com)`. Reported and originally fixed by [@psrebniak](https://github.com/psrebniak). ([#505](https://github.com/aws-samples/amazon-transcribe-live-meeting-assistant/issues/505))

- **Webex Virtual Participants act on in-meeting chat commands again.** Webex moved its meeting chat panel onto Momentum web components, so the Virtual Participant stopped seeing chat messages and commands such as `LMA leave` were ignored. Both markups are now recognized, several messages arriving together are all read rather than only the last — including a run of messages from one person, where several share a single block — and the Webex goodbye names whoever asked the Virtual Participant to leave, as it already did on Zoom and Chime. Whether the chat panel itself was found is now recorded in the Virtual Participant's log, so a future change to where Webex puts the panel shows up there rather than as chat commands that quietly do nothing. Reported and originally fixed by [@psrebniak](https://github.com/psrebniak). ([#507](https://github.com/aws-samples/amazon-transcribe-live-meeting-assistant/issues/507))

- **Google Meet is no longer described as a Virtual Participant platform.** The README, the quick-start guide, `docs/INDEX.md`, the Virtual Participant page and its in-app help panel all listed Google Meet alongside Zoom, Teams, Chime and WebEx, but the Virtual Participant has no Google Meet handler and fails immediately with "Unsupported meeting platform". Meet meetings are captured with the [Chrome extension](docs/browser-extension.md) instead, which is what those pages were describing. Several other places said the same thing and are corrected with them, including the `start_meeting_now` MCP tool's documented platform list — which its own code already rejected — the Virtual Participant column of the capture-options comparison, and two architecture diagrams. The Chrome extension's own documentation now states which platforms it covers, since it is where Meet users are sent. The meeting-invitation parser also auto-filled the create form from a Meet invitation, so the failure arrived only after the user had reviewed and submitted every other field; it now rejects the invitation up front with a message pointing at the extension, including when the invitation carries a Meet link but no platform. Adding Virtual Participant support for Google Meet is tracked in [#661](https://github.com/aws-samples/amazon-transcribe-live-meeting-assistant/issues/661).
...
- **Viewing a Virtual Participant's live desktop now requires a token minted for that participant.** Before the viewer connects, the UI calls a new `createVncEdgeToken(vpId)` mutation (or `createMicrovmVncToken(vpId)` under `VPLaunchType=MICROVM`), which confirms the caller's access to that Virtual Participant — owner, a user it was shared with, or an `Admin` — and returns a short-lived token scoped to that participant's live-view path and port. On the ECS launch types a Lambda@Edge function on the CloudFront `/vnc/*` behaviour checks the token's signature, expiry and participant before the request reaches the load balancer, and the load balancer forwards only requests carrying this deployment's own origin-verify header. The Virtual Participant's VNC server additionally requires a credential, generated on each boot and published on that participant's own record, which the mutation returns to an authorized viewer. No new parameters and no manual steps; see [Virtual Participant](docs/virtual-participant.md#how-a-viewer-is-authorized). ([#673](https://github.com/aws-samples/amazon-transcribe-live-meeting-assistant/pull/673))
  ...
  When upgrading: the parts of this that run inside the Virtual Participant container — the framebuffer's loopback binding and the origin-verify condition on the routing rule each participant creates for itself — take effect only once the container image is rebuilt, and under `MICROVM` the binding lands when the snapshot is rebuilt. Everything in CloudFormation applies as soon as the stack update completes. Participants already running keep their existing routing rule and have no credential on their record, so the viewer reaches them as before; their rules are removed by the existing cleanup when each meeting ends. See [How a viewer is authorized](docs/virtual-participant.md#how-a-viewer-is-authorized).

- **Meetings whose client stops sending activity are now closed automatically.** A meeting only left the "In Progress" state when its client sent an end-of-meeting event, so a browser tab, Desktop Capture App or browser extension session whose transcriber task went away without a clean shutdown stayed in progress for as long as its record was kept. A scheduled reaper now ends meetings that have produced no finalized transcript segment for longer than the new **`MeetingInactivityTimeoutInMinutes`** parameter (default `240`, `0` disables it) and generates their summary. The Virtual Participant and uploaded-recording paths already had their own backstops and are unchanged. Because the signal is finalized speech, a meeting that is still connected but silent for longer than the timeout is also ended — keep the value comfortably above the longest quiet stretch your meetings have. See [Meeting Stuck In Progress](docs/troubleshooting.md#meeting-stuck-in-progress).

- **Scaling controls for the WebSocket transcriber service.** Five new parameters — **`TranscriberDesiredTaskCount`**, **`TranscriberAutoScalingEnabled`**, **`TranscriberMinTaskCount`**, **`TranscriberMaxTaskCount`** and **`TranscriberScalingCpuTargetPercent`** — size the Fargate service behind the Stream Audio tab and the desktop apps, with optional CPU target-tracking scale-out. Defaults are unchanged behaviour: one task, auto scaling off. Running more than one task requires `EnableVideoRecording=false`, because meeting video arrives on a second WebSocket connection that only the task holding that meeting's audio session can attach; the combination is now rejected on the parameters page rather than failing per meeting. Scale-in is deliberately disabled, since removing a task ends the meetings on it. See [WebSocket Transcriber Service Scaling](docs/cloudformation-parameters.md#websocket-transcriber-service-scaling).
...
- **Meeting summaries are now generated from the transcript as spoken, rather than from one with filler words stripped out.** Before being sent for summarisation, transcripts had "um", "uh", "mhm" and "like" removed. That pass has been deleted. It was corrupting real speech: the pattern matched those letters wherever a word began with them and deleted only the prefix, so "umbrella" became "brella" and "Likewise" became "wise"; and because it removed every "like", meaning-bearing uses went too, turning "looks like it needs sign-off" into "looks it needs sign-off". The models summarising these transcripts handle disfluent speech without help, so the pass removed text they were going to disregard anyway while damaging text they needed. Stripping of the web UI's own markup — HTML tags and the issue-detected pill — is unchanged, since that genuinely carries no meaning. Summaries will read slightly differently after this update and should be more faithful; stored transcripts were never altered by this step and are unaffected, as is anything that does not request transcript processing. No parameters change.

- **⚠️ Deployments that use the delegated CloudFormation service role must update that role first.** This release adds the first SQS queue and the first Application Auto Scaling resources to LMA, so `iam-roles/cloudformation-management/LMA-Cloudformation-Service-Role.yaml` now grants those two services and the Application Auto Scaling service-linked role. Update the role stack from the new template before updating LMA with `--role-arn`, or the stack update fails with AccessDenied. Deployments that do not pass a service role are unaffected. See [CloudFormation Service Role](docs/cloudformation-service-role.md).
...
- **`EnableDataRetentionOnDelete` now covers everything that holds durable user data.** It previously governed some of those resources, which meant a stack deletion could remove data the setting implied it would keep — including prompt templates, chat button definitions, voice assistant settings authored from the web UI, the Cognito identity pool, and the knowledge base's vector index. All of these now follow the parameter. [Stack Updates & Upgrades](docs/stack-updates-and-upgrades.md#data-retention-on-stack-deletion) lists exactly what is retained, and also what is not: build artifacts, per-deployment secrets, short-lived handoff state and the category SNS topic are still deleted with the stack by design. ⚠️ Two notes for upgraders: deletion policies are read from the deployed template, so the new behaviour applies only after this update completes; and retained resources keep incurring storage charges until you remove them by hand.
...
- **Transcript records the pipeline cannot apply are now reported, retried and kept.** The Call Event Processor reports Kinesis partial batch failures, so the retry, bisect and record-age settings on its event source mapping apply to the records that actually failed instead of the checkpoint always advancing. Batches that are given up on, and individual records that cannot be decoded at all, are written to a new SQS queue with the shard id and sequence numbers needed to find them again — see [Discarded Transcript Records](docs/troubleshooting.md#discarded-transcript-records). One consequence of real redelivery: a record after a failing one can now be delivered more than once, so the end-of-meeting summary is skipped for a meeting that is already ended rather than being generated twice.
...
- **Rolling deployments of the transcriber service no longer drop below full capacity.** A replacement task must be healthy before an existing one drains, so the service keeps accepting meetings throughout a deployment; a deployment now briefly runs up to twice the task count instead.

- **The web application's Content-Security-Policy now names every resource type it uses.** `default-src 'self'` is the fallback, with `frame-src`, `media-src`, `font-src` and `worker-src` listed explicitly, and `script-src` allows only this distribution's own origin. If you have customised the UI to load scripts, fonts, media or workers from another origin, add that origin to the corresponding directive in `lma-ai-stack/deployment/lma-ai-stack.yaml` — a resource that is not permitted stops loading in the browser with no server-side log entry. An unmodified deployment needs no action. ([#672](https://github.com/aws-samples/amazon-transcribe-live-meeting-assistant/pull/672))

- **Markdown rendered in the web UI passes through a sanitizer.** Every markdown-to-HTML path — the eight `ReactMarkdown` call sites in the React app and the standalone chat page — now renders through a fixed allowlist of elements and attributes (`rehype-raw` then `rehype-sanitize` in the app, a vendored DOMPurify build on the chat page), and only content in that allowlist is emitted. Task-list checkboxes render in their read-only form. Ordinary markdown in transcripts, summaries and assistant replies is unchanged. The chat iframes also carry a `sandbox` attribute and their `postMessage` channel is pinned to the application origin. ([#672](https://github.com/aws-samples/amazon-transcribe-live-meeting-assistant/pull/672))
...
- **Dependency updates across every stack**, landed as grouped Dependabot batches — the web UI, WebSocket transcriber, Virtual Participant backend, browser extension, docs-site, the Python Lambda requirements, and the boto3/botocore/boto3-stubs/aws-sam-cli/cfn-policy-validator tooling family — including `js-yaml`, `hono`, `nanoid`, `devalue`, `smol-toml`, `svgo` and `vitest`, with no build regressions.
...
- **A word at a live turn cut could appear in two consecutive transcript rows on the on-demand engine.** Word timings are reconstructed from per-token timestamps, so a one-token word starts and ends on the same timestamp and was counted both as the end of one row and the start of the next — "…run through today" followed by "today does that work". Row boundaries are now partitioned on each word's end time everywhere they are applied, including text corrections that re-read a span.

- **A short opening fragment could carry the previous speaker's label on the on-demand engine.** A leading part too brief to produce a speaker embedding inherited the label from the row before it, so the first second or two of a long utterance showed as a different speaker from the rest of it. A short leading part now merges into the part that follows and is labelled by its own embedding.

- **Live transcript rows could sort above finalized rows that ended later on the on-demand engine.** A partial carried no end time, so a live row's end stayed pinned to the moment its utterance began, while the UI orders every row by end time across both channels. Partials now carry the audio time decoded so far, and reconnect offsets track partial progress with them. Amazon Transcribe partials already carried a growing end time and were unaffected.

## [0.3.8] - 2026-09-02

### Fixed

- **Scheduled Virtual Participants never launched under `VPLaunchType=MICROVM`.** The scheduler Lambda (the DynamoDB-stream path the UI's "schedule a meeting" flow uses) had one launch path: an EventBridge schedule targeting `ecs:runTask` with `VP_LAUNCH_TYPE` forwarded straight into `LaunchType`, which under `MICROVM` failed with "launch type should be one of [EC2,FARGATE,EXTERNAL]" — and because the schedule deletes itself after firing, the VP row sat at `SCHEDULED` with no error in the UI and nothing left in the console to explain it. EventBridge Scheduler has no MicroVM target, so a `MICROVM` schedule now invokes the VP MicroVM launcher Lambda directly — the same target the "start now" state machine already uses. `EC2`/`FARGATE` are unchanged. ([#613](https://github.com/aws-samples/amazon-transcribe-live-meeting-assistant/issues/613))
...
- **The Virtual Participant could not join a Teams meeting under its default display name.** Teams' pre-join name field validates against a fixed character set — letters, numbers, spaces and `- ' . _ @` — which the default identity template `LMA ({LMA_USER})` fails on the parentheses, so the join was blocked with "Your name can only include letters, numbers, spaces, and these symbols". The name is now sanitised to `LMA - user` where it is typed into the Teams form (and in the verify-and-retype comparison, which would otherwise always mismatch); chat messages, the transcript identity, every other platform and the Teams ACS SDK path keep the unmodified name.

- **The voice assistant's speech was chopped in Teams meetings** — it sounded like a square wave gating on and off 5-10 times a second, while the identical audio path was clean on Zoom. This is the open Teams audio-quality report that 0.3.7 added instrumentation for; instance size, buffers, PulseAudio and packet flow had already been ruled out by measurement, and the two remaining Teams-specific mechanisms are both fixed here. **First**, the Teams web client routes the microphone through its own noise-suppression graph and transmits *that* — recording both sides of the graph during one utterance found 0 hard cuts going in and 138 in 15 seconds coming out, 20-30 ms zero-holes at 9.2/s. Any live audio sender whose track is a `MediaStreamAudioDestinationNode` is now replaced with an unprocessed capture of the same device (the VP's "mic" is a null-sink monitor — no room, no echo, no noise to suppress), polled because Teams can swap its track back. Self-scoping by track label, so Zoom and every platform that transmits the capture track directly are no-ops by construction. **Second**, Teams negotiates Opus DTX, which makes every pause a discontinuous-transmission cycle whose exit clips the first packets of the next sentence (packet rate dipping to 10.8-32.7/s against ~50 expected at every audio onset); `usedtx=1` is now rewritten to `usedtx=0` in the remote description, costing ~14 kbps of comfort noise while idle. Known trade-off: with the raw track on the sender, the Teams client's own mute button no longer silences the agent — the VP never uses it, as agent audio is managed by the voice assistant.

- **The voice assistant could go deaf after a session refresh, and refreshed at the wrong moment in a quiet meeting.** Two defects in the `always_active` lifecycle, both observed live. The refresh gate only checked agent-side state, so the 5-minute refresh could tear down the bidirectional stream while a *user* was mid-utterance — their audio died with the old session and reached the new one only as history, which never triggers a response, so the agent appeared deaf until re-addressed and looked exactly like wake-phrase mode. User speech now stamps a last-activity time and the refresh waits for 5 seconds of quiet. Separately, Nova Sonic closes an idle stream itself at 295 s and neither meeting silence nor the 30 s keep-alive counts as interactive, so the pre-emptive refresh at 300 s lost that race in every quiet meeting, trading a planned refresh for a `ValidationException` and an unplanned one; it now runs at 240 s.
