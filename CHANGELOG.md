...
- **Virtual Participant browser stack: `puppeteer-extra-plugin-stealth` + Chrome Stable on Debian Bookworm → CloakBrowser Chromium on Debian slim** — Migrated the VP container from `node:22-bookworm-slim` + `google-chrome-stable` + `puppeteer-extra-plugin-stealth` to `python:3.12-slim` (glibc) + [CloakBrowser](https://github.com/CloakHQ/cloakbrowser) 0.3.29 (Chromium 146.0.7680.177.3). **Why:** CloakBrowser ships a Chromium binary with 32+ source-level C++ patches (canvas / WebGL / audio / fonts / GPU / WebRTC / TLS), so meeting platforms see a consistent, real-browser environment — more reliable than the previous JS-injection / CDP-runtime-patch approach, whose inconsistencies could break a join. The cloakbrowser package is `pip install`-ed and the binary is pre-downloaded at build time (`python -m cloakbrowser install`); the JS app uses `cloakbrowser/playwright`'s `launchPersistentContext`. Each user gets a stable per-Cognito-sub fingerprint seed (`fingerprintSeedForUser()`) so they present as the same returning visitor across launches. Fresh profiles run a 7-site warmup (Google → HN → Wikipedia → zoom.us / app.chime.aws / teams.microsoft.com / web.webex.com) before the meeting URL so a first-time profile arrives with normal browsing history rather than zero state. The base image is glibc (cloakbrowser's binary is glibc-only — Alpine/musl was ruled out) but trades to a smaller final image than bookworm-slim once Chrome Stable is gone. Together with the 3p-cookie pre-write fix (Chrome v123+ default-blocks 3p cookies, breaking Zoom's cross-domain auth on a fresh profile) and the persistent S3 profile, fresh-profile Zoom joins from datacenter IPs are now reliable.
...
- **Ghost-cursor for human-like mouse pathing on the Sign-In / Join / Skip clicks** — `humanClick(cursor, page, target)` helper uses `ghost-cursor` (Bezier-curve mouse movement) for the cursor-pathing-sensitive clicks (Zoom Join, Sign-in, "Skip for now" interstitials, in-meeting chat-panel button). Audio/video SVG toggles still use the pre-existing `clickClickableAncestor()` helper since SVG elements can't be the cursor target directly. More closely mimics real-user input than a bare `page.click()`, improving join reliability.
