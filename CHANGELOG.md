...
- **The on-demand MicroVM ASR engine now ships two permissively licensed bundles and nothing to tune.** The catalog is reduced to `fastconformer-titanet-small` (default: NVIDIA FastConformer streaming EN 480 ms, CC-BY-4.0, + TitaNet-small, CC-BY-4.0, + pyannote segmentation 3.0, MIT) and `fastconformer-transcription-only` (the same ASR model, no speaker weights). Every weight is redistributable; the NVIDIA-Open-Model-License Nemotron and Parakeet bundles, the Apache-2.0 Zipformer/WeSpeaker/3D-Speaker pairings and the offline Parakeet bundle are gone. The diarization operating point is baked into the image from the bundle — TitaNet-small's threshold is **0.5** (midpoint of the measured gap: the hardest real different-speaker pair scored 0.307, the same speaker at ≥2.5 s utterances 0.74–0.91; a synthetic same-gender control corroborated it at 0.539 and showed the earlier 0.4 would have merged that pair's closest utterances), minimum utterance 2500 ms. `TranscriptionEngine` is the only CloudFormation question; the `AsrDefaults` mapping keeps just `ModelBundle` and `MaxMeetingSeconds`, and the transcriber and ASR stacks lose their threshold, utterance-floor, speaker-cap, turn-cut, open-row, endpointing, engine-default and "measured" parameters. The ASR Config page is reduced to two switches — deployment default engine, and Virtual Participant voice separation — and the calibration upload, its transcriber route and the runtime tuning fields are removed; a deployment has nothing to calibrate. Developers measure a new embedder with `lma-asr-microvm-stack/source/scripts/calibrate.py` (the same statistics the page used) before recording it in the catalog. The Stream Audio engine picker now starts on the live deployment default instead of a deploy-time constant.

### Added

- **The Virtual Participant can be transcribed by the on-demand MicroVM ASR engine.** It follows the ASR Config page's deployment-default switch (per-VP `ASR_ENGINE` override for local testing), acquires and releases its own MicroVM through the same launcher the WebSocket transcriber uses, and falls back to Amazon Transcribe when no MicroVM can be acquired. Speaker names still come from the meeting roster. A second switch, **Identify separate voices behind each Virtual Participant attendee**, asks the engine for per-voice labels so several people behind one attendee tile — a conference room, or a shared screen playing a recording — come out as `Name (spk_0)`, `Name (spk_1)` instead of one name; off by default because the roster name is the better label for a normal attendee.
