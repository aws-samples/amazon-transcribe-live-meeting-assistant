...
- **Transcription stopping partway through long meetings (browser Stream Audio / extension path)** — Amazon Transcribe closes its streaming result stream server-side on its own schedule (idle behaviour, session duration limits, transient server close). The WebSocket transcriber only restarted the session from its `catch` block, so when the result stream ended *without throwing* the `for await` loop simply completed, the function returned, and transcription stopped **permanently** while the browser kept streaming audio into a dead pipe (GitHub #292). A second defect compounded it: `retryCount` was never reset after a healthy session, so ~5 transient reconnects spread across a long meeting cumulatively hit `MAX_RETRIES` and aborted with only a server-side log. `startTranscribeSession` now (a) reconnects on an unexpected clean stream close (reusing the existing `SessionId`) unless the call has legitimately ended, (b) counts only *consecutive* failures toward `MAX_RETRIES` — resetting once a session produced a transcript event or ran ≥10s, and (c) applies capped linear backoff between failed restarts. (The related "speaker always shows n/a" symptom in #292 is a separate speaker-attribution/channel-mapping issue and is tracked separately.)
