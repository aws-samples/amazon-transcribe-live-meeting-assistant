...
- Added diagnostic logging for the Virtual Participant's audio path, so the browser's microphone settings and the outgoing audio packet rate are visible in CloudWatch. This is groundwork for an unresolved report of crackly voice assistant audio on Microsoft Teams (the assistant sounds clean on Zoom).
