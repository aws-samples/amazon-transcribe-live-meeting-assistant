...
- **Delete Virtual Participants from the VP list** — New trash-can button on the Virtual Participants list page lets users bulk-delete VP records with a typed-consent confirmation (matching the meetings list UX). Active or scheduled VPs are cleanly ended first (ECS task stopped, ALB target group + listener rule removed, EventBridge schedule cancelled, Kinesis `END` emitted) before the DynamoDB record is removed. Implemented as a new `deleteVirtualParticipants` AppSync mutation backed by the existing `VirtualParticipantManager` Lambda.
...
- **Meeting links with "/" in the meeting name** — Meeting IDs are now `encodeURIComponent`-wrapped when constructing the hash route (`#/calls/<id>`, breadcrumbs, and the Meetings Query Tool's citation links), so meeting names containing `/` or other URL-reserved characters no longer break the link to the meeting detail page.
